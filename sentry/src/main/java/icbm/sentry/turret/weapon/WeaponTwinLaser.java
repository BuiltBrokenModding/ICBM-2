...
public class WeaponTwinLaser extends WeaponLaser
{
	...
	public void renderClient(Vector3 hit)
	{
		...
		ICBMSentry.proxy.renderBeam(sentry.getHost().world(), Vector3.translate(center, new Vector3(this.sentry.getHost().yaw() - 6, this.sentry.getHost().pitch() * 1.4f).scale(1.2)), hit, 1, 0.4f, 0.4f, 5);
		ICBMSentry.proxy.renderBeam(sentry.getHost().world(), Vector3.translate(center, new Vector3(this.sentry.getHost().yaw() + 6, this.sentry.getHost().pitch() * 1.4f).scale(1.2)), hit, 1, 0.4f, 0.4f, 5);
