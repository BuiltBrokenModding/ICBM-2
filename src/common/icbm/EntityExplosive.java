...
public class EntityExplosive extends Entity implements IRotatable, IEntityAdditionalSpawnData
{
    ...
    public EntityExplosive(World par1World, Vector3 position, byte orientation, int explosiveID)
    {
        ...
        this.fuse = Explosive.list[explosiveID].getFuse();
        this.orientation = orientation;
        
        Explosive.list[explosiveID].preDetontation(par1World, this);        
    }
    
    public EntityExplosive(World par1World, Vector3 position, int explosiveID, byte orientation, int metadata)
    {
    ...
    {   
        ...
        if(this.fuse < 1)
        {
            ...
            Explosive.createExplosion(this.worldObj, Vector3.get(this), this, this.explosiveID);
            this.setDead();
        }
        else
        {
        	Explosive.list[explosiveID].onDetonating(this.worldObj, new Vector3(this.posX, this.posY, this.posZ), this.fuse);
        }
    ...
    {
    	this.fuse = Explosive.list[explosiveID].onDestroyedByExplosion();
    }
