...
public class BlockInvisible extends BlockContainer
{
    ...
    {
		worldObj.setBlockWithNotify(position.intX(), position.intY(), position.intZ(), ICBM.blockInvisible.blockID);
		((TileEntityInvisibleBlock)worldObj.getBlockTileEntity(position.intX(), position.intY(), position.intZ())).setMainBlock(mainBlock);
    }

    @Override
    public void breakBlock(World par1World, int x, int y, int z, int par5, int par6)
    {
    	TileEntityInvisibleBlock tileEntity = (TileEntityInvisibleBlock)par1World.getBlockTileEntity(x, y, z);
	...
	{
    	return new TileEntityInvisibleBlock();
	}
    ...
    public ItemStack getPickBlock(MovingObjectPosition target, World par1World, int x, int y, int z)
    {
		...
		Vector3 mainBlockPosition = ((TileEntityInvisibleBlock)tileEntity).mainBlockPosition;
