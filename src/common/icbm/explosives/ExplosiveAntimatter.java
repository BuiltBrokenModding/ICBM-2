...
public class ExplosiveAntimatter extends Explosive
{	
	public boolean destroyBedrock = false;
	
	public ExplosiveAntimatter(String name, int ID, int tier)
	{
		super(name, ID, tier);
		this.setFuse(300);
	...
	public boolean doExplosion(World worldObj, Vector3 position, Entity source, int radius)
	{
        ...
        List<EntityProceduralExplosion> allEntities = worldObj.getEntitiesWithinAABB(EntityProceduralExplosion.class, bounds);
    	
        for(EntityProceduralExplosion entity : allEntities)
        {            
