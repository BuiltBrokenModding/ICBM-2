...
public class PoisonRadiation extends Poison {
	...
	public static final CustomDamageSource damageSource = (CustomDamageSource) new CustomDamageSource(
			"radiation").setDamageBypassesArmor();
		...
		if (!PoisonRadiation.disabled) {
			if (emitPosition == null) {
				entity.addPotionEffect(new CustomPotionEffect(
						...
						20 * 15 * (amplifier + 1), amplifier, null));
				return;
			}
			...
			if (this.getAntiRadioactiveCount(entity.worldObj, emitPosition,
				...
				entity.addPotionEffect(new CustomPotionEffect(
						PotionRadiation.INSTANCE.getId(),
						20 * 20 * (amplifier + 1), amplifier, null));
			}
		}
	}
	...
	public int getAntiRadioactiveCount(World world, Vector3 startingPosition,
		...
		Vector3 delta = Vector3.subtract(endingPosition, startingPosition)
				.normalize();
			...
			while (targetPosition.distanceTo(endingPosition) <= totalDistance) {
				...
				if (blockID > 0) {
					if (Block.blocksList[blockID] instanceof IAntiPoisonBlock) {
						if (((IAntiPoisonBlock) Block.blocksList[blockID])
								.isPoisonPrevention(world,
										targetPosition.intX(),
										targetPosition.intY(),
