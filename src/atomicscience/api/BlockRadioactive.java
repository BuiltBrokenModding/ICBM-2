...
public class BlockRadioactive extends Block {
	...
	public void registerIcons(IconRegister iconRegister) {
		...
		this.iconTop = iconRegister.registerIcon(this.getUnlocalizedName()
				.replace("tile.", "") + "_top");
		this.iconBottom = iconRegister.registerIcon(this.getUnlocalizedName()
				.replace("tile.", "") + "_bottom");
	}
	...
	public void updateTick(World world, int x, int y, int z, Random rand) {
		if (!world.isRemote) {
			if (this.isRandomlyRadioactive) {
				AxisAlignedBB bounds = AxisAlignedBB.getBoundingBox(x
						- this.radius, y - this.radius, z - this.radius, x
						+ this.radius, y + this.radius, z + this.radius);
				List<EntityLivingBase> entitiesNearby = world
						.getEntitiesWithinAABB(EntityLivingBase.class, bounds);

				for (EntityLivingBase entity : entitiesNearby) {
					PoisonRadiation.INSTANCE.poisonEntity(new Vector3(x, y, z),
							entity, amplifier);
				}
			}
			...
			if (this.canSpread) {
				for (int i = 0; i < 4; ++i) {
					...
					if (rand.nextFloat() > 0.4
							&& (blockID == Block.tilledField.blockID || blockID == Block.grass.blockID)) {
						world.setBlockMetadataWithNotify(newX, newY, newZ,
								this.blockID, 2);
					}
				}
		...
		if (this.spawnParticle) {
			if (Minecraft.getMinecraft().gameSettings.particleSetting == 0) {
				...
				for (int i = 0; i < 2; i++) {
					...
					EntitySmokeFX fx = new EntitySmokeFX(world, diDian.x,
							diDian.y, diDian.z, (Math.random() - 0.5) / 2,
							(Math.random() - 0.5) / 2,
							(Math.random() - 0.5) / 2);
