...
public class ItShouLiuDan extends ItICBM {
	...
	public ItemStack onItemRightClick(ItemStack itemStack, World world,
		...
		if (itemStack != null) {
			...
			ExplosivePreDetonationEvent evt = new ExplosivePreDetonationEvent(
					world, entityPlayer, ExplosiveType.ITEM, zhaPin);
			...
			if (!evt.isCanceled()) {
				...
				entityPlayer.sendChatToPlayer(ChatMessageComponent
						.func_111066_d("Grenades are banned in this region."));
			}
		}
	...
	public void onPlayerStoppedUsing(ItemStack itemStack, World world,
			EntityPlayer entityPlayer, int nengLiang) {
		if (!world.isRemote) {
			ZhaPin zhaPin = ZhaPinRegistry.get(itemStack.getItemDamage());
			ExplosivePreDetonationEvent evt = new ExplosivePreDetonationEvent(
					world, entityPlayer, ExplosiveType.ITEM, zhaPin);
			...
			if (!evt.isCanceled()) {
				...
				world.spawnEntityInWorld(new EShouLiuDan(world, entityPlayer,
						zhaPin.getID(), (float) (this
								.getMaxItemUseDuration(itemStack) - nengLiang)
	...
	public void registerIcons(IconRegister iconRegister) {
		for (int i = 0; i < ZhaPinRegistry.getAllZhaPin().size(); i++) {
			ICONS[i] = iconRegister.registerIcon(ZhuYaoICBM.PREFIX + "grenade_"
					+ ZhaPinRegistry.get(i).getUnlocalizedName());
		}
	}
