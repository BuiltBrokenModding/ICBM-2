...
public class BZhaDan extends BICBM implements ICamouflageMaterial {
	 ...
	 */
	private static byte determineOrientation(World world, int x, int y, int z,
			EntityLivingBase entityLiving) {
		if (entityLiving != null) {
			if (MathHelper.abs((float) entityLiving.posX - x) < 2.0F
					&& MathHelper.abs((float) entityLiving.posZ - z) < 2.0F) {
			...
			return (byte) (rotation == 0 ? 2 : (rotation == 1 ? 5
	...
	public AxisAlignedBB getCollisionBoundingBoxFromPool(World par1World,
		...
		if (tileEntity != null) {
			if (tileEntity instanceof TZhaDan) {
				if (((TZhaDan) tileEntity).haoMa == ZhaPin.diLei.getID()) {
					return AxisAlignedBB.getAABBPool().getAABB(x + this.minX,
							y + this.minY, z + this.minZ, x + this.maxX,
							y + 0.2, z + this.maxZ);
				}
			}
		}
	...
	public void onBlockPlacedBy(World world, int x, int y, int z,
		...
		((TZhaDan) world.getBlockTileEntity(x, y, z)).haoMa = itemStack
				.getItemDamage();
		int explosiveID = ((TZhaDan) world.getBlockTileEntity(x, y, z)).haoMa;

		if (!world.isRemote) {
			if (ZhuYaoZhaPin.shiBaoHu(world, new Vector3(x, y, z),
					ExplosiveType.BLOCK, explosiveID)) {
		...
		world.setBlockMetadataWithNotify(
				...
				VectorHelper.getOrientationFromSide(
						ForgeDirection.getOrientation(determineOrientation(
								world, x, y, z, entityLiving)),
						ForgeDirection.NORTH).ordinal(), 2);
		...
		if (entityLiving != null) {
			FMLLog.fine(entityLiving.getEntityName() + " placed "
					+ ZhaPinRegistry.get(explosiveID).getExplosiveName()
					+ " in: " + x + ", " + y + ", " + z + ".");
		}
	}
	...
	public Icon getBlockTexture(IBlockAccess par1IBlockAccess, int x, int y,
			int z, int side) {
		int explosiveID = ((TZhaDan) par1IBlockAccess.getBlockTileEntity(x, y,
				z)).haoMa;
	...
	public void registerIcons(IconRegister iconRegister) {
		...
		for (ZhaPin zhaPin : ZhaPinRegistry.getAllZhaPin()) {
			ICON_TOP[zhaPin.getID()] = this.getIcon(iconRegister, zhaPin,
					"_top");
			ICON_SIDE[zhaPin.getID()] = this.getIcon(iconRegister, zhaPin,
					"_side");
			ICON_BOTTOM[zhaPin.getID()] = this.getIcon(iconRegister, zhaPin,
					"_bottom");
		}
	}
	...
	public Icon getIcon(IconRegister iconRegister, ZhaPin zhaPin, String suffix) {
		...
		try {
			ResourceLocation resourcelocation = new ResourceLocation(
					ZhuYaoICBM.DOMAIN, ZhuYaoICBM.BLOCK_PATH + iconName
							+ ".png");
			InputStream inputstream = Minecraft.getMinecraft().func_110442_L()
					.func_110536_a(resourcelocation).func_110527_b();
		...
		if (suffix.equals("_bottom")) {
			return iconRegister.registerIcon(ZhuYaoICBM.PREFIX
					+ "explosive_bottom_" + zhaPin.getTier());
		}

		return iconRegister.registerIcon(ZhuYaoICBM.PREFIX + "explosive_base_"
				+ zhaPin.getTier());
	}
	...
	/*
	 * Called to detonate the TNT. Args: world, x, y, z, metaData,
	 * CauseOfExplosion (0, intentional, 1, exploded, 2 burned)
	 */
	public static void yinZha(World world, int x, int y, int z,
			int explosiveID, int causeOfExplosion) {
		if (!world.isRemote) {
			if (!ZhuYaoZhaPin.shiBaoHu(world, new Vector3(x, y, z),
					ExplosiveType.BLOCK, explosiveID)) {
				...
				if (tileEntity != null) {
					if (tileEntity instanceof TZhaDan) {
						...
						EZhaDan eZhaDan = new EZhaDan(world, new Vector3(x, y,
								z).add(0.5), ((TZhaDan) tileEntity).haoMa,
								(byte) world.getBlockMetadata(x, y, z),
								((TZhaDan) tileEntity).nbtData);
	...
	public boolean onBlockActivated(World world, int x, int y, int z,
		...
		if (entityPlayer.getCurrentEquippedItem() != null) {
			if (entityPlayer.getCurrentEquippedItem().itemID == Item.flintAndSteel.itemID) {
				...
				BZhaDan.yinZha(world, x, y, z, explosiveID, 0);
				return true;
			} else if (this.isUsableWrench(entityPlayer,
					entityPlayer.getCurrentEquippedItem(), x, y, z)) {
				byte change = 3;
				...
				world.setBlockMetadataWithNotify(x, y, z, ForgeDirection
						.getOrientation(change).ordinal(), 3);
		...
		if (tileEntity instanceof TZhaDan) {
			return ZhaPinRegistry.get(((TZhaDan) tileEntity).haoMa)
					.onBlockActivated(world, x, y, z, entityPlayer, par6, par7,
							par8, par9);
		}
		...
		if (tileEntity != null) {
			if (tileEntity instanceof TZhaDan) {
				if (!((TZhaDan) tileEntity).exploding) {
					int explosiveID = ((TZhaDan) tileEntity).haoMa;
					int id = idDropped(world.getBlockMetadata(x, y, z),
							world.rand, 0);

					this.dropBlockAsItem_do(world, x, y, z, new ItemStack(id,
							1, explosiveID));
				}
			}
		}
