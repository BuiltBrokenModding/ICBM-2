...
public class ExTuPuo extends ZhaPin
{
	...
	public void doBaoZha(World worldObj, Vector3 position, Entity explosionSource)
	{
		if (!worldObj.isRemote)
		{
			...
			for (int i = 0; i < 8; i++)
			{
				...
				{
					if (Block.blocksList[targetPosition.getBlockID(worldObj)].getExplosionResistance(explosionSource, worldObj, targetPosition.intX(), targetPosition.intY(), targetPosition.intZ(), position.x, position.y, position.z) > Block.obsidian.getExplosionResistance(explosionSource) || Block.blocksList[targetPosition.getBlockID(worldObj)] instanceof IForceFieldBlock)
					{
						break;
					}
				}
	...
	public void init()
	{
		RecipeHelper.addRecipe(new ShapedOreRecipe(this.getItemStack(2), new Object[] { "GCG", "GCG", "GCG", 'C', yaSuo.getItemStack(), 'G', Item.gunpowder }), this.getUnlocalizedName(), ZhuYaoBase.CONFIGURATION, true);
	}
