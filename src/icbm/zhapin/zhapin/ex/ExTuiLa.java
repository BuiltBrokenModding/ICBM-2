...
public class ExTuiLa extends DaoDan {
	...
	public void doCreateExplosion(World world, double x, double y, double z,
			Entity entity) {
		if (this.getID() == ZhaPin.la.getID()) {
			new BzYaSuo(world, entity, x, y, z, 2f).setDestroyItems()
					.setPushType(1).explode();
		} else {
			new BzYaSuo(world, entity, x, y, z, 2f).setDestroyItems()
					.setPushType(2).explode();
