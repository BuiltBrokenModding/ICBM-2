...
public class ExQunDan extends DaoDan {
	...
	public void init() {
		if (this.getID() == ZhaPin.xiaoQunDan.getID()) {
			RecipeHelper.addRecipe(new ShapedOreRecipe(this.getItemStack(),
					new Object[] { "???", "?@?", "???", '@',
							tui.getItemStack(), '?', Item.arrow }), this
					.getUnlocalizedName(), SheDing.CONFIGURATION, true);
		} else if (this.getID() == ZhaPin.zhen.getID()) {
			RecipeHelper.addRecipe(new ShapedOreRecipe(this.getItemStack(10),
					new Object[] { "SSS", "SAS", "SSS", 'A', Block.anvil, 'S',
							ZhaPin.xiaoQunDan.getItemStack() }), this
					.getUnlocalizedName(), SheDing.CONFIGURATION, true);
		} else if (this.getID() == ZhaPin.qunDan.getID()) {
			RecipeHelper.addRecipe(
					new ShapedOreRecipe(this.getItemStack(), new Object[] {
							" @ ", "@?@", " @ ", '?', huo.getItemStack(), '@',
							xiaoQunDan.getItemStack() }),
	...
	public void doCreateExplosion(World world, double x, double y, double z,
			Entity entity) {
		if (this.getTier() == 2) {
			new BzQunDan(world, entity, x, y, z, 15, true, true, false)
					.explode();
		} else if (this.getID() == ZhaPin.zhen.getID()) {
			new BzQunDan(world, entity, x, y, z, 25, false, false, true)
					.explode();
		} else {
			new BzQunDan(world, entity, x, y, z, 30, true, false, false)
					.explode();
		}
	}
