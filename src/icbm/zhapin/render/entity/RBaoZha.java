...
public class RBaoZha extends Render {
	public static final ResourceLocation TEXTURE_FILE = new ResourceLocation(
			ZhuYaoICBM.DOMAIN, ZhuYaoICBM.TEXTURE_PATH + "blackhole.png");
	...
	public void doRender(Entity entity, double x, double y, double z,
		...
		if (eZhaPin.baoZha != null) {
			...
			if (eZhaPin.baoZha instanceof BzHongSu) {
				 ...
				 * 
				 * int textureSize = 50; float size4 = size * 5; float
				 * float_sizeMinus0_01 = textureSize - 0.01F;
				 * 
				 * float x0 = (textureSize + 0.0F) / size4; float x1 =
				 * (textureSize + float_sizeMinus0_01) / size4; float x2 =
				 * (textureSize + 0.0F) / size4; float x3 = (textureSize +
				 * float_sizeMinus0_01) / size4;
				 * 
				 * float renderX = (float) x; float renderY = (float) y; float
				 * renderZ = (float) z;
				 * 
				...
				for (int i1 = 0; i1 < (var41 + var41 * var41) / 2.0F * 60.0F; ++i1) {
					...
					GL11.glRotatef(rand.nextFloat() * 360.0F, 1.0F, 0.0F, 0.0F);
					GL11.glRotatef(rand.nextFloat() * 360.0F, 0.0F, 1.0F, 0.0F);
					GL11.glRotatef(rand.nextFloat() * 360.0F + var41 * 90.0F,
							0.0F, 0.0F, 1.0F);
					tessellator.startDrawing(6);
					float var81 = rand.nextFloat() * 20.0F + 5.0F + var51
							* 10.0F;
					float var91 = rand.nextFloat() * 2.0F + 1.0F + var51 * 2.0F;
					tessellator.setColorRGBA_I(16777215,
							(int) (255.0F * (1.0F - var51)));
					tessellator.addVertex(0.0D, 0.0D, 0.0D);
					tessellator.setColorRGBA_I(0, 0);
					tessellator
							.addVertex(-0.866D * var91, var81, -0.5F * var91);
				...
				if (eZhaPin.baoZha.getRenderModel() != null
						&& eZhaPin.baoZha.getRenderResource() != null) {
					GL11.glPushMatrix();
					...
					eZhaPin.baoZha.getRenderModel().render(eZhaPin, (float) x,
							(float) y, (float) z, par8, par9, 0.0625F);
					GL11.glPopMatrix();
				}
			}
		}
