		...
		if (entityMissile.getExplosiveType() instanceof DaoDan) {
			...
			GL11.glRotatef(
					entityMissile.prevRotationYaw
							+ (entityMissile.rotationYaw - entityMissile.prevRotationYaw)
							* f1 - 90.0F, 0.0F, 1.0F, 0.0F);
			GL11.glRotatef(
					entityMissile.prevRotationPitch
							+ (entityMissile.rotationPitch - entityMissile.prevRotationPitch)
							* f1 + 90.0F, 0.0F, 0.0F, 1.0F);
			...
			FMLClientHandler.instance().getClient().renderEngine
					.func_110577_a(daoDan.getMissileResource());
			daoDan.getMuoXing().render(entityMissile, (float) x, (float) y,
					(float) z, f, f1, 0.0625F);
