...
public class BJiQi extends BICBM {
	public enum JiQi {
		FaSheDi(TFaSheDi.class), FaSheShiMuo(TFaSheShiMuo.class), FaSheJia(
				TFaSheJia.class), LeiDaTai(TLeiDaTai.class), DianCiQi(
				TDianCiQi.class), XiaoFaSheQi(TXiaoFaSheQi.class), YinDaoQi(
				TYinDaoQi.class);
	...
	public static boolean canBePlacedAt(World world, int x, int y, int z,
			int metadata, int direction) {
		switch (metadata) {
		default: {
		...
		case 3: {
			...
			return world.getBlockMaterial(x, y - 1, z).isSolid()
					...
					&& world.getBlockId(x, y + 1, z + 1) == 0
					&& world.getBlockId(x, y + 1, z - 1) == 0
					&& world.getBlockId(x + 1, y + 1, z + 1) == 0
					&& world.getBlockId(x - 1, y + 1, z - 1) == 0
					&& world.getBlockId(x + 1, y + 1, z - 1) == 0
					&& world.getBlockId(x - 1, y + 1, z + 1) == 0;
	...
	public boolean onMachineActivated(World par1World, int x, int y, int z,
		...
		if (tileEntity != null) {
			if (tileEntity instanceof IBlockActivate) {
				return ((IBlockActivate) tileEntity)
						.onActivated(par5EntityPlayer);
			}
		}
	...
	public void breakBlock(World par1World, int x, int y, int z, int par5,
		...
		if (tileEntity != null) {
			...
			this.dropBlockAsItem_do(par1World, x, y, z, new ItemStack(
					ZhuYaoZhaPin.bJiQi, 1, itemMetadata));
	...
	@Override
	public ItemStack getPickBlock(MovingObjectPosition target, World par1World,
			int x, int y, int z) {
		TileEntity tileEntity = par1World.getBlockTileEntity(x, y, z);
