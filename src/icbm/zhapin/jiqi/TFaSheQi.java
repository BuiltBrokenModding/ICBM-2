...
public abstract class TFaSheQi extends TileEntityUniversalElectrical implements
		ILauncherController, IPeripheral, IRedstoneReceptor {
	...
	public Object[] callMethod(IComputerAccess computer, ILuaContext context,
		...
		case 2:
			if (arguments[0] != null && arguments[1] != null
					&& arguments[2] != null) {
				try {
					this.setTarget(new Vector3(((Double) arguments[0])
							.doubleValue(), ((Double) arguments[1])
							.doubleValue(), ((Double) arguments[2])
							.doubleValue()));
				} catch (Exception e) {
					e.printStackTrace();
		...
		case 4:
			if (arguments[0] != null) {
				try {
					...
					arg = Math.max(Math.min(arg, Short.MAX_VALUE),
							Short.MIN_VALUE);
		...
		case 6:
			if (this.getMissile() != null) {
				return new Object[] { this.getMissile().getExplosiveType()
						.getMissileName() };
			} else {
				return null;
			}
