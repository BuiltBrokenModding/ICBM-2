...
public class TXiaoFaSheQi extends TFaSheQi implements IBlockActivate,
		IPacketReceiver, IInventory, ILauncherContainer {
	...
	public void updateEntity() {
		...
		this.receiveElectricity(null, ElectricityPack.getFromWatts(
				ElectricItemHelper.dischargeItem(this.containingItems[1],
						this.getRequest(null)), this.getVoltage()), true);
	...
	public Packet getDescriptionPacket() {
		return PacketManager.getPacket(ZhuYaoZhaPin.CHANNEL, this, 0,
				this.getEnergyStored(), this.shengBuo, this.muBiao.x,
				this.muBiao.y, this.muBiao.z);
	}
	...
	private float getPitchFromTarget() {
		double distance = Math.sqrt((this.muBiao.x - this.xCoord)
				* (this.muBiao.x - this.xCoord) + (this.muBiao.z - this.zCoord)
				* (this.muBiao.z - this.zCoord));
		return (float) Math.toDegrees(Math
				.atan((this.muBiao.y - (this.yCoord + 0.5F)) / distance));
	}
	...
	public boolean canLaunch() {
		if (this.daoDan != null && this.containingItems[0] != null) {
			DaoDan missile = (DaoDan) ZhaPinRegistry
					.get(this.containingItems[0].getItemDamage());

			if (missile != null
					&& missile.getID() == daoDan.getExplosiveType().getID()
					&& missile.isCruise() && missile.getTier() <= 3) {
				if (this.getEnergyStored() >= this.getMaxEnergyStored()) {
	...
	public boolean isTooClose(Vector3 target) {
		...
		if (Vector3.distance(new Vector3(this.xCoord, 0, this.zCoord),
				new Vector3(target.x, 0, target.z)) < 8) {
			return true;
		}
	...
	public boolean onActivated(EntityPlayer entityPlayer) {
		...
		entityPlayer.openGui(ZhuYaoZhaPin.instance,
				ZhuYaoICBM.GUI_XIA_FA_SHE_QI, this.worldObj, this.xCoord,
