...
public class IBJiQi extends ItemBlock {
	...
	public boolean placeBlockAt(ItemStack itemStack, EntityPlayer entityPlayer,
		...
		int direction = MathHelper
				.floor_double((entityPlayer.rotationYaw * 4.0F / 360.0F) + 0.5D) & 3;
			...
			if (world.setBlock(x, y, z, this.getBlockID(), jiQiMetadata, 3)) {
				if (world.getBlockId(x, y, z) == this.getBlockID()) {
					if (itemStack.getItemDamage() < 9) {
						ITier tileEntity = (ITier) world.getBlockTileEntity(x,
								y, z);
						...
						if (tileEntity != null) {
							if (itemStack.getItemDamage() < 3) {
								...
								tileEntity
										.setTier(itemStack.getItemDamage() - 3);
							} else if (itemStack.getItemDamage() < 9) {
								tileEntity
										.setTier(itemStack.getItemDamage() - 6);
							}
						}
					}

					Block.blocksList[this.getBlockID()].onBlockPlacedBy(world,
							x, y, z, entityPlayer, itemStack);
				}
