...
public class TLeiDaTai extends TileEntityUniversalElectrical implements
		IChunkLoadHandler, IPacketReceiver, IRedstoneProvider, IMultiBlock,
		IPeripheral {
	...
	public void initiate() {
		this.worldObj
				.notifyBlocksOfNeighborChange(this.xCoord, this.yCoord,
						this.zCoord, this.worldObj.getBlockId(this.xCoord,
								this.yCoord, this.zCoord));
		this.chunkLoaderInit(ForgeChunkManager.requestTicket(
				ZhuYaoZhaPin.instance, this.worldObj, Type.NORMAL));
	}
	...
	public void chunkLoaderInit(Ticket ticket) {
		if (!this.worldObj.isRemote) {
			if (this.ticket == null && ticket != null) {
				...
				ForgeChunkManager.forceChunk(this.ticket,
						new ChunkCoordIntPair(this.xCoord >> 4,
								this.zCoord >> 4));
			}
		}
	}
	...
	public void updateEntity() {
		...
		try {
			...
			if (this.electricityHandler.provideElectricity(
					this.getRequest(null), true).getWatts() >= this
					.getRequest(null)) {
	...
	private void doScan() {
		...
		List<Entity> entities = RadarRegistry.getEntitiesWithinRadius(
				new Vector3(this).toVector2(), MAX_BIAN_JING);
		...
		List<EntityPlayer> players = this.worldObj.getEntitiesWithinAABB(
				EntityPlayer.class, AxisAlignedBB.getBoundingBox(this.xCoord
						- MAX_BIAN_JING, this.yCoord - MAX_BIAN_JING,
						this.zCoord - MAX_BIAN_JING, this.xCoord
								+ MAX_BIAN_JING, this.yCoord + MAX_BIAN_JING,
						this.zCoord + MAX_BIAN_JING));
		...
		for (TileEntity jiQi : RadarRegistry.getTileEntitiesInArea(new Vector2(
				this.xCoord - TLeiDaTai.MAX_BIAN_JING, this.zCoord
						- TLeiDaTai.MAX_BIAN_JING), new Vector2(this.xCoord
				+ TLeiDaTai.MAX_BIAN_JING, this.zCoord
				+ TLeiDaTai.MAX_BIAN_JING))) {
	...
	public boolean isWeiXianDaoDan(EDaoDan daoDan) {
		...
		return (Vector2.distance(new Vector3(daoDan).toVector2(), new Vector2(
				this.xCoord, this.zCoord)) < this.alarmBanJing && Vector2
				.distance(daoDan.muBiao.toVector2(), new Vector2(this.xCoord,
						this.zCoord)) < this.safetyBanJing);
	}

	private Packet getDescriptionPacket2() {
		return PacketManager.getPacket(ZhuYaoZhaPin.CHANNEL, this, 1,
				this.alarmBanJing, this.safetyBanJing);
	}

	@Override
	public Packet getDescriptionPacket() {
		return PacketManager.getPacket(ZhuYaoZhaPin.CHANNEL, this, 4,
		...
		try {
			...
			if (ID == -1) {
				if (dataStream.readBoolean()) {
					PacketManager.sendPacketToClients(
							this.getDescriptionPacket2(), this.worldObj,
							new Vector3(this), 15);
						...
						double dist = Vector2
								.distance(
										...
										new Vector2(
												this.xCoord
														+ ForgeDirection
																.getOrientation(i).offsetX,
												this.zCoord
														+ ForgeDirection
																.getOrientation(i).offsetZ));
	...
	public void onDestroy(TileEntity callingBlock) {
		...
		this.worldObj.setBlock(this.xCoord + 1, this.yCoord + 1,
				this.zCoord + 1, 0, 0, 2);
		this.worldObj.setBlock(this.xCoord - 1, this.yCoord + 1,
				this.zCoord - 1, 0, 0, 2);
		this.worldObj.setBlock(this.xCoord + 1, this.yCoord + 1,
	...
	public boolean onActivated(EntityPlayer entityPlayer) {
		if (entityPlayer.inventory.getCurrentItem() != null) {
			if (((BlockAdvanced) this.getBlockType()).isUsableWrench(
					entityPlayer, entityPlayer.inventory.getCurrentItem(),
					this.xCoord, this.yCoord, this.zCoord)) {
				if (!this.worldObj.isRemote) {
					this.emitAll = !this.emitAll;
					entityPlayer
							.addChatMessage("Radar redstone all side emission: "
									+ this.emitAll);
				}
		...
		entityPlayer.openGui(ZhuYaoZhaPin.instance, ZhuYaoICBM.GUI_LEI_DA_TAI,
	...
	public void onCreate(Vector3 position) {
		ZhuYaoICBM.bJia.makeFakeBlock(worldObj,
				Vector3.add(new Vector3(0, 1, 0), position), new Vector3(this));

		ZhuYaoICBM.bJia.makeFakeBlock(worldObj,
				Vector3.add(new Vector3(1, 1, 0), position), new Vector3(this));
		ZhuYaoICBM.bJia
				.makeFakeBlock(worldObj,
						Vector3.add(new Vector3(-1, 1, 0), position),
						new Vector3(this));

		ZhuYaoICBM.bJia.makeFakeBlock(worldObj,
				Vector3.add(new Vector3(0, 1, 1), position), new Vector3(this));
		ZhuYaoICBM.bJia
				.makeFakeBlock(worldObj,
						Vector3.add(new Vector3(0, 1, -1), position),
						new Vector3(this));

		ZhuYaoICBM.bJia
				.makeFakeBlock(worldObj,
						Vector3.add(new Vector3(1, 1, -1), position),
						new Vector3(this));
		ZhuYaoICBM.bJia
				.makeFakeBlock(worldObj,
						Vector3.add(new Vector3(-1, 1, 1), position),
		...
		ZhuYaoICBM.bJia.makeFakeBlock(worldObj,
				Vector3.add(new Vector3(1, 1, 1), position), new Vector3(this));
		ZhuYaoICBM.bJia.makeFakeBlock(worldObj,
				Vector3.add(new Vector3(-1, 1, -1), position),
				new Vector3(this));
	...
	public Object[] callMethod(IComputerAccess computer, ILuaContext context,
		...
		case 0:
			List<Entity> entities = RadarRegistry.getEntitiesWithinRadius(
					new Vector3(this).toVector2(), this.alarmBanJing);
		...
		case 1:
			for (TileEntity jiQi : RadarRegistry.getTileEntitiesInArea(
					new Vector2(this.xCoord - TLeiDaTai.MAX_BIAN_JING,
							this.zCoord - TLeiDaTai.MAX_BIAN_JING),
					new Vector2(this.xCoord + TLeiDaTai.MAX_BIAN_JING,
							this.zCoord + TLeiDaTai.MAX_BIAN_JING))) {
