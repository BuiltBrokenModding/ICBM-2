...
public class GDianCiQi extends GuiBase {
	...
	public void initGui() {
		...
		this.buttonList.add(new GuiButton(0, this.width / 2 - 77,
				this.height / 2 - 10, 50, 20, "Missiles"));
		this.buttonList.add(new GuiButton(1, this.width / 2 - 25,
				this.height / 2 - 10, 65, 20, "Electricity"));
	...
	public void keyTyped(char par1, int par2) {
		...
		try {
			int radius = Math.min(Math.max(
					Integer.parseInt(this.textFieldBanJing.getText()), 10),
					TDianCiQi.MAX_RADIUS);
			this.tileEntity.banJing = radius;
			PacketDispatcher.sendPacketToServer(PacketManager.getPacket(
					ZhuYaoZhaPin.CHANNEL, this.tileEntity, 2,
					this.tileEntity.banJing));
	...
	public void mouseClicked(int par1, int par2, int par3) {
		...
		this.textFieldBanJing.mouseClicked(par1 - containerWidth, par2
	...
	protected void drawForegroundLayer(int var2, int var3, float var1) {
		...
		this.fontRenderer.drawString("EMP Radius:          blocks", 12, 30,
				4210752);
		...
		this.fontRenderer.drawString(color + "Status: " + status, 12, 120,
				4210752);
