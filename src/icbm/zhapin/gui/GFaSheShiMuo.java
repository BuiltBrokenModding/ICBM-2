...
public class GFaSheShiMuo extends GuiBase {
	...
	public void initGui() {
		...
		PacketDispatcher.sendPacketToServer(PacketManager.getPacket(
				ZhuYaoZhaPin.CHANNEL, this.tileEntity, -1, true));
	}
	...
	public void onGuiClosed() {
		super.onGuiClosed();
		PacketDispatcher.sendPacketToServer(PacketManager.getPacket(
				ZhuYaoZhaPin.CHANNEL, this.tileEntity, -1, false));
	}
	...
	public void keyTyped(char par1, int par2) {
		...
		try {
			Vector3 newTarget = new Vector3(
					Integer.parseInt(this.tFX.getText()), Math.max(
							Integer.parseInt(this.tFY.getText()), 0),
		...
		try {
			short newFrequency = (short) Math.max(
					Short.parseShort(this.tFFreq.getText()), 0);
		...
		try {
			short newGaoDu = (short) Math.max(Math.min(
					Short.parseShort(this.tFGaoDu.getText()), Short.MAX_VALUE),
					3);

			this.tileEntity.gaoDu = newGaoDu;
			PacketDispatcher.sendPacketToServer(PacketManager.getPacket(
					ZhuYaoZhaPin.CHANNEL, this.tileEntity, 3,
					this.tileEntity.gaoDu));
		} catch (NumberFormatException e) {
	...
	public void mouseClicked(int par1, int par2, int par3) {
		...
		this.tFX.mouseClicked(par1 - containerWidth, par2 - containerHeight,
				par3);
		this.tFZ.mouseClicked(par1 - containerWidth, par2 - containerHeight,
				par3);

		if (tileEntity.getTier() >= 1) {
			this.tFY.mouseClicked(par1 - containerWidth,
					par2 - containerHeight, par3);
			this.tFGaoDu.mouseClicked(par1 - containerWidth, par2
					- containerHeight, par3);

			if (tileEntity.getTier() > 1) {
				this.tFFreq.mouseClicked(par1 - containerWidth, par2
						- containerHeight, par3);
			}
		}
	...
	public void drawForegroundLayer(int var2, int var3, float var1) {
		...
		this.fontRenderer.drawString("\u00a77Launcher Control Panel", 30, 6,
				4210752);
		...
		this.fontRenderer.drawString("Inaccuracy: " + inaccuracy + " blocks",
				12, 113, 4210752);

		// Shows the status of the missile launcher
		this.fontRenderer.drawString("Status: " + this.tileEntity.getStatus(),
				12, 125, 4210752);
		this.fontRenderer.drawString("Voltage: " + this.tileEntity.getVoltage()
				+ "v", 12, 137, 4210752);
		this.fontRenderer.drawString(
				ElectricityDisplay.getDisplayShort(
						...
						+ ElectricityDisplay.getDisplayShort(
								...
								ElectricUnit.JOULES), 12, 150, 4210752);
	}
