...
public abstract class BaoZha extends Explosion implements IExplosion {
	...
	public BaoZha(World world, Entity entity, double x, double y, double z,
			float size) {
	...
	@Override
	public void explode() {
		ExplosionConstructionEvent evt = new ExplosionConstructionEvent(
				this.worldObj, this);
		MinecraftForge.EVENT_BUS.post(evt);
	...
	protected void doDamageEntities(float radius, float power,
		...
		List<Entity> allEntities = worldObj.getEntitiesWithinAABB(Entity.class,
				AxisAlignedBB.getBoundingBox(minCoord.intX(), minCoord.intY(),
						minCoord.intZ(), maxCoord.intX(), maxCoord.intY(),
						maxCoord.intZ()));
		Vec3 var31 = Vec3
				.createVectorHelper(position.x, position.y, position.z);
		...
		for (int i = 0; i < allEntities.size(); ++i) {
			...
			double distance = entity.getDistance(position.x, position.y,
					position.z) / radius;
			...
			if (distance <= 1.0D) {
				...
				double var35 = MathHelper
						.sqrt_double(xDifference * xDifference + yDifference
								* yDifference + zDifference * zDifference);
				xDifference /= var35;
				yDifference /= var35;
				zDifference /= var35;
				double var34 = worldObj.getBlockDensity(var31,
						entity.boundingBox);
	...
	/**
	 * Called by doDamageEntity on each entity being damaged. This function
	 * should be inherited if something special is to happen to a specific
	 * entity.
	 * 
