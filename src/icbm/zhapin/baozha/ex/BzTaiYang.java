...
public class BzTaiYang extends BzGuang
{
	...
	{
		super.doExplode();
		this.worldObj.playSoundEffect(position.x, position.y, position.z, "icbm.beamcharging", 4.0F, 0.8F);
	}
	...
	public void doPostExplode()
	{
		...
		{
			this.worldObj.playSoundEffect(position.x, position.y, position.z, "icbm.powerdown", 4.0F, 0.8F);
			...
			if (this.canFocusBeam(this.worldObj, position) && this.thread.isComplete)
			{
				...
				this.worldObj.playSoundEffect(position.x + 0.5D, position.y + 0.5D, position.z + 0.5D, "icbm.explosionfire", 6.0F, (1.0F + (worldObj.rand.nextFloat() - worldObj.rand.nextFloat()) * 0.2F) * 1F);
			}
