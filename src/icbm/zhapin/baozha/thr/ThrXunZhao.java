			...
			for (int x = 0; x < this.banJing; ++x) {
				for (int y = 0; y < this.banJing; ++y) {
					for (int z = 0; z < this.banJing; ++z) {
						if (x == 0 || x == this.banJing - 1 || y == 0
								|| y == this.banJing - 1 || z == 0
								|| z == this.banJing - 1) {
							double xStep = x / (this.banJing - 1.0F) * 2.0F
									- 1.0F;
							double yStep = y / (this.banJing - 1.0F) * 2.0F
									- 1.0F;
							double zStep = z / (this.banJing - 1.0F) * 2.0F
									- 1.0F;
							...
							for (float var21 = 0.3F; power > 0.0F; power -= var21 * 0.75F) {
								Vector3 targetPosition = new Vector3(var15,
										var17, var19);
								double distanceFromCenter = position
										.distanceTo(targetPosition);
								int blockID = this.world.getBlockId(
										targetPosition.intX(),
										targetPosition.intY(),
										targetPosition.intZ());
								...
								if (blockID > 0) {
									...
									if (blockID == Block.bedrock.blockID) {
										...
										resistance = Block.blocksList[blockID]
