...
public class BzShengBuo extends BaoZha {
	...
	public BzShengBuo(World world, Entity entity, double x, double y, double z,
			float size) {
	...
	public void doPreExplode() {
		if (!this.worldObj.isRemote) {
			if (this.hasShockWave) {
				for (int x = (int) (-this.getRadius() * 2); x < this
						.getRadius() * 2; ++x) {
					for (int y = (int) (-this.getRadius() * 2); y < this
							.getRadius() * 2; ++y) {
						for (int z = (int) (-this.getRadius() * 2); z < this
								.getRadius() * 2; ++z) {
							Vector3 targetPosition = Vector3.add(position,
									new Vector3(x, y, z));
							int blockID = worldObj.getBlockId(
									targetPosition.intX(),
									targetPosition.intY(),
									targetPosition.intZ());
								...
								if (blockID != Block.bedrock.blockID
										...
										&& (Block.blocksList[blockID]
												.getExplosionResistance(
														...
														position.intX(),
														position.intY(),
														position.intZ()) > this.nengLiang || material == Material.glass)) {
						...
						public float getResistance(World world,
							...
							if (block instanceof BlockFluid
								...
								resistance = block.getExplosionResistance(
										source, world, targetPosition.intX(),
										targetPosition.intY(),
										targetPosition.intZ(),
										explosionPosition.intX(),
										explosionPosition.intY(),
										explosionPosition.intZ());
							}
		...
		if (!this.worldObj.isRemote) {
			if (this.thread.isComplete) {
				...
				while (it.hasNext()) {
					...
					int metadata = this.worldObj.getBlockMetadata(
							targetPosition.intX(), targetPosition.intY(),
							targetPosition.intZ());
					...
					if (distance < r - 1 || this.worldObj.rand.nextInt(3) > 0) {
						if (blockID == ZhuYaoZhaPin.bZhaDan.blockID) {
							BZhaDan.yinZha(
									...
									targetPosition.intY(),
									targetPosition.intZ(),
									((TZhaDan) this.worldObj
											.getBlockTileEntity(
													targetPosition.intX(),
													targetPosition.intY(),
													targetPosition.intZ())).haoMa,
									1);
						} else {
							this.worldObj.setBlock(targetPosition.intX(),
									targetPosition.intY(),
									targetPosition.intZ(), 0, 0, 2);
						}

						targetPosition.add(0.5D);

						if (this.worldObj.rand.nextFloat() < 0.3 * (this
								.getRadius() - r)) {
							EFeiBlock entity = new EFeiBlock(this.worldObj,
									targetPosition, blockID, metadata);
							this.worldObj.spawnEntityInWorld(entity);
							entity.yawChange = 50 * this.worldObj.rand
									.nextFloat();
							entity.pitchChange = 100 * this.worldObj.rand
									.nextFloat();
						}
