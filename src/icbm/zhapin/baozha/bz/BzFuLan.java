...
public class BzFuLan extends BaoZha {
	...
	public BzFuLan(World world, Entity entity, double x, double y, double z,
			float size) {
	...
	public void doPreExplode() {
		if (!this.worldObj.isRemote) {
			this.thread = new ThrSheXian(worldObj, position,
					...
					if (blockID > 0) {
						...
						if (blockID == Block.stone.blockID) {
							...
							if (FluidRegistry.getFluid("toxicwaste") != null) {
								targetPosition.setBlock(this.worldObj,
										FluidRegistry.getFluid("toxicwaste")
												.getBlockID());
							}
						}
					}
