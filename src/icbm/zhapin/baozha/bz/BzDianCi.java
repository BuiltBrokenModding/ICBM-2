...
public class BzDianCi extends BaoZha {
	...
	public BzDianCi(World world, Entity entity, double x, double y, double z,
			float size) {
	...
	public void doExplode() {
		if (this.effectBlocks) {
			for (int x = (int) -this.getRadius(); x < (int) this.getRadius(); x++) {
				for (int y = (int) -this.getRadius(); y < (int) this
						.getRadius(); y++) {
					for (int z = (int) -this.getRadius(); z < (int) this
							.getRadius(); z++) {
						double dist = MathHelper.sqrt_double((x * x + y * y + z
								* z));

						Vector3 searchPosition = Vector3.add(position,
								new Vector3(x, y, z));
		...
		if (this.effectEntities) {
			// Drop all missiles
			List<Entity> entitiesNearby = RadarRegistry
					.getEntitiesWithinRadius(position.toVector2(),
							(int) this.getRadius());

			for (Entity entity : entitiesNearby) {
				if (entity instanceof IMissile
						&& !entity.isEntityEqual(this.controller)) {
		...
		ZhuYaoZhaPin.proxy.spawnParticle("shockwave", worldObj, position, 0, 0,
				0, 0, 0, 255, 10, 3);
		this.worldObj
				.playSoundEffect(position.x, position.y, position.z,
						ZhuYaoICBM.PREFIX + "emp", 4.0F,
