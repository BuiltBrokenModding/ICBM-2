...
public class BzFanWuSu extends BaoZha {
	...
	public BzFanWuSu(World world, Entity entity, double x, double y, double z,
			float size) {
		super(world, entity, x, y, z, size);
	}

	public BzFanWuSu(World world, Entity entity, double x, double y, double z,
			float size, boolean destroyBedrock) {
	...
	public void doPreExplode() {
		...
		this.worldObj.playSoundEffect(this.position.x, this.position.y,
				this.position.z, ZhuYaoICBM.PREFIX + "antimatter", 7F,
				(float) (this.worldObj.rand.nextFloat() * 0.1 + 0.9F));
	...
	public void doExplode() {
		...
		/*
		 * else if (ZhuYao.proxy.isGaoQing()) { for (int x = -this.getRadius();
		 * x < this.getRadius(); x++) { for (int y = -this.getRadius(); y <
		 * this.getRadius(); y++) { for (int z = -this.getRadius(); z <
		 * this.getRadius(); z++) { Vector3 targetPosition =
		 * Vector3.add(position, new Vector3(x, y, z)); double distance =
		 * position.distanceTo(targetPosition);
		 * 
		 * if (targetPosition.getBlockID(worldObj) == 0) { if (distance <
		 * this.getRadius() && distance > this.getRadius() - 1 &&
		 * worldObj.rand.nextFloat() > 0.5) {
		 * ParticleSpawner.spawnParticle("antimatter", worldObj,
		 * targetPosition); } } } } } }
		 */
	}
