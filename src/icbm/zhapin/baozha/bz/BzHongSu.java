...
public class BzHongSu extends BaoZha {
	...
	public BzHongSu(World world, Entity entity, double x, double y, double z,
			float size) {
	...
	public void doPreExplode() {
		if (!this.worldObj.isRemote) {
			this.worldObj.createExplosion(this.exploder, position.x,
					position.y, position.z, 5.0F, true);
		}
	}
	...
	public void doExplode() {
		...
		if (!this.worldObj.isRemote) {
			...
			loop: for (int r = 1; r < this.getRadius(); r++) {
				for (int x = -r; x < r; x++) {
					for (int y = -r; y < r; y++) {
						for (int z = -r; z < r; z++) {
							...
							if (block != null) {
								if (block instanceof IForceFieldBlock) {
									((IForceFieldBlock) block)
											.weakenForceField(this.worldObj,
													currentPos.intX(),
													currentPos.intY(),
													currentPos.intZ(), 50);
									continue;
								...
								if (block.getBlockHardness(this.worldObj,
										currentPos.intX(), currentPos.intY(),
										currentPos.intZ()) <= -1)
									continue;
								...
								if (this.worldObj.rand.nextFloat() > 0.8) {
									EFeiBlock entity = new EFeiBlock(
											this.worldObj, currentPos, blockID,
											metadata);
									this.worldObj.spawnEntityInWorld(entity);
									entity.yawChange = 50 * this.worldObj.rand
											.nextFloat();
		...
		AxisAlignedBB bounds = AxisAlignedBB.getBoundingBox(
				...
				position.x + radius, position.y + radius, position.z + radius);
		List<Entity> allEntities = this.worldObj.getEntitiesWithinAABB(
				Entity.class, bounds);
		 ...
		 * ZhuYaoZhaPin.proxy.getParticleSetting()); i++) { Vector3 randomVector
		 * = new Vector3(this.worldObj.rand.nextInt((int) this.getRadius()) -
		 * this.getRadius(), this.worldObj.rand.nextInt((int) this.getRadius())
		 * - this.getRadius(), this.worldObj.rand.nextInt((int)
		 * this.getRadius()) - this.getRadius());
		 * ZhuYaoZhaPin.proxy.spawnParticle("smoke", this.worldObj,
		 * Vector3.add(this.position, randomVector), 0, 0, 0, 1, 1, 1, 7.0F, 8);
		 * }
		 * 
		 * List<Entity> list = ZhuYaoZhaPin.proxy.getEntityFXs();
		 * 
		 * if (list != null) { for (Entity entity : list) { if
		 * (this.position.distanceTo(new Vector3(entity)) <= radius) {
		 * this.affectEntity(radius, entity, false); } } } }
		 */

		if (this.worldObj.rand.nextInt(8) == 0) {
			this.worldObj.playSoundEffect(position.x + (Math.random() - 0.5)
					* radius, position.y + (Math.random() - 0.5) * radius,
					position.z + (Math.random() - 0.5) * radius,
					ZhuYaoICBM.PREFIX + "collapse",
					6.0F - this.worldObj.rand.nextFloat(),
					1.0F - this.worldObj.rand.nextFloat() * 0.4F);
		}

		this.worldObj.playSoundEffect(position.x, position.y, position.z,
				ZhuYaoICBM.PREFIX + "redmatter", 3.0F,
	...
	public boolean affectEntity(float radius, Entity entity, boolean doExplosion) {
		...
		double distancePercentage = 2 - (this.position.distanceTo(new Vector3(
				entity)) / radius);

		Vector3 entityPosition = new Vector3(entity);
		Vector3 centeredPosition = entityPosition.clone().subtract(
				this.position);
		CalculationHelper.rotateByAngle(centeredPosition, 6
				* distancePercentage * Math.random(), 6 * distancePercentage
				* Math.random(), 6 * distancePercentage * Math.random());
		Vector3 newPosition = Vector3.add(this.position, centeredPosition);
		entity.motionX /= 2;
		entity.motionY /= 2;
		entity.motionZ /= 2;
		// Orbit Velocity
		entity.addVelocity(newPosition.x - entityPosition.x, 0, newPosition.z
				- entityPosition.z);
		// Gravity Velocity
		entity.addVelocity(-xDifference * 0.04 * xPercentage, -yDifference
				* 0.04 * yPercentage, -zDifference * 0.04 * zPercentage);

		if (this.worldObj.isRemote) {
			if (entity instanceof EFeiBlock) {
				if (ZhuYaoZhaPin.proxy.getParticleSetting() == 0) {
					if (this.worldObj.rand.nextInt(5) == 0) {
						ZhuYaoZhaPin.proxy.spawnParticle("digging",
								this.worldObj, new Vector3(entity),
								-xDifference, -yDifference + 10, -zDifference,
								((EFeiBlock) entity).blockID, 0,
								((EFeiBlock) entity).metadata, 2, 1);
		...
		if (Vector3.distance(
				new Vector3(entity.posX, entity.posY, entity.posZ), position) < 4) {
			if (doExplosion && !explosionCreated && callCount % 5 == 0) {
				this.worldObj.createExplosion(this.exploder, entity.posX,
						entity.posY, entity.posZ, 3.0F, true);
			...
			if (entity instanceof EntityLiving) {
				...
				if (entity instanceof EBaoZha) {
					if (((EBaoZha) entity).baoZha instanceof BzFanWuSu
							|| ((EBaoZha) entity).baoZha instanceof BzHongSu) {
						this.worldObj
								.playSoundEffect(
										position.x,
										position.y,
										position.z,
										ZhuYaoICBM.PREFIX + "explosion",
										7.0F,
										(1.0F + (this.worldObj.rand.nextFloat() - this.worldObj.rand
												.nextFloat()) * 0.2F) * 0.7F);

						if (this.worldObj.rand.nextFloat() > 0.85
