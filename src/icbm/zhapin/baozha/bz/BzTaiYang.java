...
public class BzTaiYang extends BzGuang {
	public BzTaiYang(World world, Entity entity, double x, double y, double z,
			float size) {
	...
	public void doExplode() {
		...
		this.worldObj.playSoundEffect(position.x, position.y, position.z,
				ZhuYaoICBM.PREFIX + "beamcharging", 4.0F, 0.8F);
	}
	...
	public void doPostExplode() {
		...
		if (!this.worldObj.isRemote) {
			this.worldObj.playSoundEffect(position.x, position.y, position.z,
					ZhuYaoICBM.PREFIX + "powerdown", 4.0F, 0.8F);
			...
			if (this.canFocusBeam(this.worldObj, position)
				...
				for (Vector3 targetPosition : this.thread.results) {
					...
					if (chance > distanceFromCenter * 0.55) {
						/*
						 * Check to see if the block is an air block and there
						 * is a block below it to support the fire.
						 */
						...
						if ((blockID == 0 || blockID == Block.snow.blockID)
							...
							if (this.worldObj.rand.nextFloat() > 0.999) {
								...
								if (((ExTaiYang) ZhaPin.taiYang).createNetherrack
										&& (blockID == Block.stone.blockID
												|| blockID == Block.grass.blockID || blockID == Block.dirt.blockID)
										&& this.worldObj.rand.nextFloat() > 0.75) {
							...
							this.worldObj.setBlockToAir(targetPosition.intX(),
									targetPosition.intY(),
									targetPosition.intZ());
						}
					}
	...
	public boolean canFocusBeam(World worldObj, Vector3 position) {
		...
		return worldTime < 12000 && !worldObj.isRaining()
				&& super.canFocusBeam(worldObj, position);
	}
