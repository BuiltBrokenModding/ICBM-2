...
public class BzQunDan extends BaoZha {
	...
	public BzQunDan(World world, Entity entity, double x, double y, double z,
			float size, boolean isFlaming, boolean isExplosive, boolean isAnvil) {
		...
		if (!worldObj.isRemote) {
			...
			for (int i = 0; i < this.getRadius(); i++) {
				...
				for (int ii = 0; ii < this.getRadius(); ii++) {
					ESuiPian arrow = new ESuiPian(worldObj, position.x,
							position.y, position.z, this.isExplosive,
							this.isAnvil);
					...
					arrow.setLocationAndAngles(position.x, position.y,
							position.z, rotationYaw, rotationPitch);
					arrow.posX -= (MathHelper.cos(rotationYaw / 180.0F
							* (float) Math.PI) * 0.16F);
					arrow.posY -= 0.10000000149011612D;
					arrow.posZ -= (MathHelper.sin(rotationYaw / 180.0F
							* (float) Math.PI) * 0.16F);
					arrow.setPosition(arrow.posX, arrow.posY, arrow.posZ);
					arrow.yOffset = 0.0F;
					arrow.motionX = (-MathHelper.sin(rotationYaw / 180.0F
							* (float) Math.PI) * MathHelper.cos(rotationPitch
							/ 180.0F * (float) Math.PI));
					arrow.motionZ = (MathHelper.cos(rotationYaw / 180.0F
							* (float) Math.PI) * MathHelper.cos(rotationPitch
							/ 180.0F * (float) Math.PI));
					arrow.motionY = (-MathHelper.sin(rotationPitch / 180.0F
							* (float) Math.PI));
