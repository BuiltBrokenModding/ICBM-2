...
public class BzBingDan extends BzGuang {
	public BzBingDan(World world, Entity entity, double x, double y, double z,
			float size) {
	...
	public void doExplode() {
		...
		this.worldObj.playSoundEffect(position.x, position.y, position.z,
				ZhuYaoICBM.PREFIX + "redmatter", 4.0F, 0.8F);
	}
	...
	public void doPostExplode() {
		...
		if (!this.worldObj.isRemote) {
			if (this.canFocusBeam(this.worldObj, position)
				 ...
				 */
				List<EntityLiving> livingEntities = worldObj
						.getEntitiesWithinAABB(EntityLiving.class,
								AxisAlignedBB.getBoundingBox(position.x
										- getRadius(),
										position.y - getRadius(), position.z
												- getRadius(), position.x
												+ getRadius(), position.y
												+ getRadius(), position.z
												+ getRadius()));
				...
				while (it.hasNext()) {
					...
					entity.addPotionEffect(new CustomPotionEffect(
							PDongShang.INSTANCE.getId(), 60 * 20, 1, null));
					entity.addPotionEffect(new PotionEffect(
							Potion.confusion.id, 10 * 20, 2));
					entity.addPotionEffect(new PotionEffect(
							Potion.digSlowdown.id, 120 * 20, 2));
					entity.addPotionEffect(new PotionEffect(
							Potion.moveSlowdown.id, 120 * 20, 4));
				}
				...
				for (Vector3 targetPosition : this.thread.results) {
					...
					double distanceFromCenter = position
					...
					/*
					 * Reduce the chance of setting blocks on fire based on
					 * distance from center.
					 */
					double chance = this.getRadius()
							- (Math.random() * distanceFromCenter);
					...
					if (chance > distanceFromCenter * 0.55) {
						...
						if (blockID == Block.fire.blockID
							...
							this.worldObj.setBlock(targetPosition.intX(),
									targetPosition.intY(),
									targetPosition.intZ(), Block.snow.blockID,
									0, 2);
						} else if (blockID == 0
								&& this.worldObj.getBlockId(
										targetPosition.intX(),
										targetPosition.intY() - 1,
										targetPosition.intZ()) != Block.ice.blockID
								&& worldObj.getBlockId(targetPosition.intX(),
										targetPosition.intY() - 1,
										targetPosition.intZ()) != 0) {
							this.worldObj.setBlock(targetPosition.intX(),
									targetPosition.intY(),
									targetPosition.intZ(), Block.ice.blockID,
									0, 2);
				...
				this.worldObj.playSoundEffect(position.x + 0.5D,
						position.y + 0.5D, position.z + 0.5D, ZhuYaoICBM.PREFIX
								+ "redmatter", 6.0F,
