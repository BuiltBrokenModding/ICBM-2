...
public class BzPiaoFu extends BaoZha {
	...
	public BzPiaoFu(World world, Entity entity, double x, double y, double z,
			float size) {
	...
	public void doPreExplode() {
		...
		this.worldObj
				.playSoundEffect(position.x, position.y, position.z,
						ZhuYaoICBM.PREFIX + "antigravity", 6.0F,
	...
	public void doExplode() {
		...
		if (!this.worldObj.isRemote && this.thread.isComplete) {
			...
			for (Vector3 targetPosition : this.thread.results) {
				...
				int blockID = worldObj.getBlockId(targetPosition.intX(),
				...
				int metadata = worldObj.getBlockMetadata(targetPosition.intX(),
						targetPosition.intY(), targetPosition.intZ());

				if (distance < r - 1 || worldObj.rand.nextInt(3) > 0) {
					worldObj.setBlock(targetPosition.intX(),
							targetPosition.intY(), targetPosition.intZ(), 0, 0,
							2);

					targetPosition.add(0.5D);

					if (worldObj.rand.nextFloat() < 0.3 * (this.getRadius() - r)) {
						EFeiBlock entity = new EFeiBlock(worldObj,
								targetPosition, blockID, metadata, 0);
						worldObj.spawnEntityInWorld(entity);
						feiBlocks.add(entity);
						entity.yawChange = 50 * worldObj.rand.nextFloat();
						entity.pitchChange = 100 * worldObj.rand.nextFloat();
						entity.motionY += Math.max(
								0.15 * worldObj.rand.nextFloat(), 0.1);
					}
