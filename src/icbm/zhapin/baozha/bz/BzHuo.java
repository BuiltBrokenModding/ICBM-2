...
public class BzHuo extends BaoZha {
	public BzHuo(World world, Entity entity, double x, double y, double z,
			float size) {
	...
	public void doExplode() {
				...
				for (int z = 0; z < radius; ++z) {
					if (x == 0 || x == radius - 1 || y == 0 || y == radius - 1
						...
						for (float var21 = 0.3F; var14 > 0.0F; var14 -= var21 * 0.75F) {
							Vector3 targetPosition = new Vector3(var15, var17,
									var19);
							double distanceFromCenter = position
									.distanceTo(targetPosition);
							int var25 = worldObj.getBlockId(
									targetPosition.intX(),
									targetPosition.intY(),
									targetPosition.intZ());
							...
							if (var14 > 0.0F) {
								...
								double chance = radius
										- (Math.random() * distanceFromCenter);

								if (chance > distanceFromCenter * 0.55) {
									/*
									 * Check to see if the block is an air block
									 * and there is a block below it to support
									 * the fire.
									 */
		...
		worldObj.playSoundEffect(
				...
				position.y + 0.5D,
				position.z + 0.5D,
				ZhuYaoICBM.PREFIX + "explosionfire",
				4.0F,
