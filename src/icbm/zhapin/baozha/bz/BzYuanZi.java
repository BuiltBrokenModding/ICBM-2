...
public class BzYuanZi extends BaoZha {
	...
	static {
		...
		POLLUTIVE_NUCLEAR = SheDing.CONFIGURATION.get(
				Configuration.CATEGORY_GENERAL, "Pollutive Nuclear",
				POLLUTIVE_NUCLEAR).getBoolean(POLLUTIVE_NUCLEAR);
	...
	public BzYuanZi(World world, Entity entity, double x, double y, double z,
			float size) {
		super(world, entity, x, y, z, size);
	}

	public BzYuanZi(World world, Entity entity, double x, double y, double z,
			float size, float nengLiang) {
	...
	public void doPreExplode() {
		if (!this.worldObj.isRemote) {
			this.thread = new ThrSheXian(this.worldObj, this.position,
					(int) this.getRadius(), this.nengLiang, this.exploder);
			this.thread.run();
				...
				for (int x = -r; x < r; x++) {
					for (int z = -r; z < r; z++) {
						...
						if (r > distance && r - 3 < distance) {
							Vector3 spawnPosition = Vector3.add(position,
									new Vector3(x * 2, (y - 2) * 2, z * 2));
							float xDiff = (float) (spawnPosition.x - position.x);
							float zDiff = (float) (spawnPosition.z - position.z);
							ZhuYaoZhaPin.proxy.spawnParticle("smoke", worldObj,
									spawnPosition,
									xDiff * 0.3 * worldObj.rand.nextFloat(),
									-worldObj.rand.nextFloat(), zDiff * 0.3
											* worldObj.rand.nextFloat(),
									(float) (distance / this.getRadius())
											* worldObj.rand.nextFloat(), 0, 0,
									8F, 1.2F);
						}
					}
				}
		...
		this.worldObj.playSoundEffect(this.position.x, this.position.y,
				this.position.z, ZhuYaoICBM.PREFIX + "explosion", 7.0F,
				...
				for (int x = -r; x < r; x++) {
					for (int z = -r; z < r; z++) {
						...
						if (distance < r && distance > r - 1) {
							Vector3 targetPosition = Vector3.add(this.position,
									new Vector3(x, 0, z));

							if (this.worldObj.rand.nextFloat() < Math.max(
									0.001 * r, 0.05)) {
								ZhuYaoZhaPin.proxy.spawnParticle("smoke",
										this.worldObj, targetPosition, 5F, 1F);
							}
						}
					}
				}
	...
	public void doPostExplode() {
		try {
			if (!this.worldObj.isRemote && this.thread.isComplete) {
				for (Vector3 targetPosition : this.thread.results) {
					int blockID = this.worldObj.getBlockId(
							targetPosition.intX(), targetPosition.intY(),
							targetPosition.intZ());

					if (blockID > 0) {

						Block.blocksList[blockID].onBlockExploded(
								this.worldObj, targetPosition.intX(),
			...
			ZhuYaoICBM.LOGGER.severe("Nuclear-type detonation Failed!");
			e.printStackTrace();
		}
		...
		if (this.isRadioactive) {
			new BzFuLan(worldObj, this.exploder, position.x, position.y,
					position.z, this.getRadius(), this.nengLiang).explode();
		...
		this.worldObj.playSoundEffect(this.position.x, this.position.y,
				this.position.z, ZhuYaoICBM.PREFIX + "explosion", 10.0F,
				(1.0F + (this.worldObj.rand.nextFloat() - this.worldObj.rand
						.nextFloat()) * 0.2F) * 0.7F);
	}
	 ...
	 * 
	 * @param return - Return -1 if this explosive does not need procedural
	 *        calls
	 */
	@Override
