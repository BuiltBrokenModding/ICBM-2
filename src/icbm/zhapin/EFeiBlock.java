...
public class EFeiBlock extends Entity implements IEntityAdditionalSpawnData {
	...
	public void onUpdate() {
		...
		if (this.posY > 400 || Block.blocksList[this.blockID] == null
				|| this.blockID == ZhuYaoICBM.bJia.blockID
				|| this.blockID == Block.pistonExtension.blockID
				|| this.blockID == Block.waterMoving.blockID
				|| this.blockID == Block.lavaMoving.blockID) {
			this.setDead();
			return;
		}
		...
		if (par1Entity instanceof EntityLiving) {
			if (Block.blocksList[this.blockID] != null) {
				if (!(Block.blocksList[this.blockID] instanceof BlockFluid)
						&& (this.motionX > 2 || this.motionY > 2 || this.motionZ > 2)) {
					int damage = (int) (1.2 * (Math.abs(this.motionX)
							+ Math.abs(this.motionY) + Math.abs(this.motionZ)));
					((EntityLiving) par1Entity).attackEntityFrom(
							DamageSource.fallingBlock, damage);
				}
			}
		}
