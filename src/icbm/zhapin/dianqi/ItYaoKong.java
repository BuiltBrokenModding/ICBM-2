		...
		if (entityPlayer.isSneaking() && tileEntity != null) {
			if (this.nengZha(tileEntity)) {
				...
				if (this.getElectricityStored(itemStack) > YONG_DIAN_LIANG) {
					...
					if (world.isRemote) {
						entityPlayer.addChatMessage("Remote Locked to: X:" + x
								+ ", Y:" + y + ", Z:" + z);
					}
	...
	public ItemStack onItemRightClick(ItemStack itemStack, World world,
		...
		if (world.isRemote) {
			if (!entityPlayer.isSneaking()) {
				MovingObjectPosition objectMouseOver = entityPlayer.rayTrace(
						BAN_JING, 1);
				...
				if (objectMouseOver != null
					...
					int blockID = world.getBlockId(objectMouseOver.blockX,
						...
						if (tile instanceof TZhaDan) {
							if (blockID == ZhuYaoZhaPin.bJiQi.blockID) {
								...
								if (this.getElectricityStored(itemStack) > YONG_DIAN_LIANG) {
									...
									entityPlayer
											.addChatMessage("Remote out of electricity!");
								}
							}
						}
					}
				...
				if (this.getElectricityStored(itemStack) > YONG_DIAN_LIANG) {
					TileEntity tileEntity = this.getSavedCoord(itemStack)
							.getTileEntity(world);

					if (this.nengZha(tileEntity)) {
						// Blow it up
						PacketDispatcher.sendPacketToServer(PacketManager
								.getPacket(ZhuYaoZhaPin.CHANNEL, tileEntity,
										(byte) 2));
						// Use Energy
						PacketDispatcher.sendPacketToServer(PacketManager
								.getPacketWithID(ZhuYaoZhaPin.CHANNEL,
										ZhaPinPacketType.REMOTE.ordinal()));
					}
				} else {
					entityPlayer.addChatMessage("Remote out of electricity!");
				}
	...
	public boolean nengZha(TileEntity tileEntity) {
		if (tileEntity != null) {
			if (tileEntity instanceof TZhaDan) {
				return ((TZhaDan) tileEntity).haoMa == ZhaPin.yaSuo.getID()
						|| ((TZhaDan) tileEntity).haoMa == ZhaPin.tuPuo.getID()
						|| ((TZhaDan) tileEntity).haoMa == ZhaPin.diLei.getID();
			}
		}
