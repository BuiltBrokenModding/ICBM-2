					...
					if (inventoryStack != null) {
						if (inventoryStack.getItem() instanceof ItDaoDan) {
							...
							if (ZhaPinRegistry.get(haoMa) instanceof DaoDan) {
								DaoDan daoDan = (DaoDan) ZhaPinRegistry
										.get(haoMa);

								if (daoDan != null
										&& !ZhuYaoZhaPin.shiBaoHu(world,
												new Vector3(player),
												ExplosiveType.AIR, haoMa)) {
									// Limit the missile to tier two.
									if (daoDan.getTier() <= 2
											&& daoDan.isCruise()) {
										double dist = 5000;
										Vector3 diDian = Vector3.add(
												new Vector3(player),
												new Vector3(0, 0.5, 0));
										Vector3 kan = new Vector3(
												player.getLook(1));
										Vector3 kaiShiDiDian = Vector3.add(
												diDian,
												Vector3.multiply(kan, 1.1));
										Vector3 muBiao = Vector3.add(diDian,
												Vector3.multiply(kan, 100));

										EDaoDan eDaoDan = new EDaoDan(world,
												kaiShiDiDian, daoDan.getID(),
												player.rotationYaw,
										...
										this.discharge(itemStack,
												YONG_DIAN_LIANG, true);

										return itemStack;
									}
								} else {
									player.sendChatToPlayer(ChatMessageComponent
											.func_111066_d("Region being is protected."));
								}
							}
