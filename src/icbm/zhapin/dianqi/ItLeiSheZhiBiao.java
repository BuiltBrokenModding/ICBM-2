...
public class ItLeiSheZhiBiao extends ItElectricICBM implements IItemFrequency {
	...
	public void onUpdate(ItemStack par1ItemStack, World par2World,
		...
		if (!par2World.isRemote) {
			...
			if (this.getLauncherCountDown(par1ItemStack) > 0
				...
				Vector3 position = new Vector3(par3Entity.posX,
						par3Entity.posY, par3Entity.posZ);
				List<TFaSheQi> launchers = FaSheQiGuanLi.naFaSheQiInArea(
						new Vector2(position.x - ItLeiSheZhiBiao.BAN_JING,
								position.z - ItLeiSheZhiBiao.BAN_JING),
						new Vector2(position.x + ItLeiSheZhiBiao.BAN_JING,
								position.z + ItLeiSheZhiBiao.BAN_JING));

				for (TFaSheQi missileLauncher : launchers) {
					if (missileLauncher != null
							&& missileLauncher.getFrequency() == this
									.getFrequency(par1ItemStack)) {
			...
			if (this.getLauncherCountDown(par1ItemStack) > 0
				...
				if (this.getLauncherCountDown(par1ItemStack) % 20 == 0) {
					((EntityPlayer) par3Entity)
							.addChatMessage("Calling air strike in: "
									+ (int) Math.floor(this
											.getLauncherCountDown(par1ItemStack) / 20));
				}

				if (this.getLauncherCountDown(par1ItemStack) == 1) {
					this.setLauncherCount(par1ItemStack,
							connectedLaunchers.size());
					this.setLauncherDelay(par1ItemStack, 0);
					((EntityPlayer) par3Entity)
							.addChatMessage("Incoming air strike!");
				}
			...
			if (this.getLauncherCount(par1ItemStack) > 0
					&& this.getLauncherCount(par1ItemStack) <= connectedLaunchers
							.size() && connectedLaunchers.size() > 0) {
				// Launch a missile every two seconds from different launchers
				if (this.getLauncherDelay(par1ItemStack) % 40 == 0) {
					connectedLaunchers.get(
							this.getLauncherCount(par1ItemStack) - 1).launch();
					this.setLauncherCount(par1ItemStack,
			...
			if (tileEntity != null) {
				if (tileEntity instanceof TFaSheQi) {
					...
					if (missileLauncher.getFrequency() > 0) {
						this.setFrequency(missileLauncher.getFrequency(),
								par1ItemStack);
						par2EntityPlayer
								.addChatMessage("Laser designator frequency Set: "
										+ this.getFrequency(par1ItemStack));
					} else {
						par2EntityPlayer
								.addChatMessage("Frequency must be greater than zero.");
					}
				}
			}
		}
	...
	public ItemStack onItemRightClick(ItemStack par1ItemStack, World par2World,
		...
		if (par2World.isRemote) {
			MovingObjectPosition objectMouseOver = par3EntityPlayer.rayTrace(
					BAN_JING * 2, 1);
			...
			if (objectMouseOver != null
				...
				// then blow it up.
				int blockId = par2World.getBlockId(objectMouseOver.blockX,
						objectMouseOver.blockY, objectMouseOver.blockZ);
				int blockMetadata = par2World.getBlockMetadata(
				...
				if (blockId == ZhuYaoZhaPin.bJiQi.blockID) {
					...
					if (airStrikeFreq > 0) {
						if (this.getElectricityStored(par1ItemStack) > YONG_DIAN_LIANG) {
							...
							List<TFaSheQi> launchers = FaSheQiGuanLi
									.naFaSheQiInArea(
											new Vector2(
													position.x
															- ItLeiSheZhiBiao.BAN_JING,
													position.z
															- ItLeiSheZhiBiao.BAN_JING),
											new Vector2(
													position.x
							...
							if (doAirStrike
								...
								PacketDispatcher
										.sendPacketToServer(PacketManager
												.getPacketWithID(
														ZhuYaoZhaPin.CHANNEL,
														ZhaPinPacketType.LASER_DESIGNATOR
																.ordinal(),
														objectMouseOver.blockX,
														objectMouseOver.blockY,
														objectMouseOver.blockZ));
								par3EntityPlayer
										.addChatMessage("Calling air strike into designated position!");
							}
						} else {
							par3EntityPlayer
									.addChatMessage("Laser designator out of electricity!");
						}
					} else {
						par3EntityPlayer
								.addChatMessage("Laser designator frequency not set!");
					}
				}
			}
		}
