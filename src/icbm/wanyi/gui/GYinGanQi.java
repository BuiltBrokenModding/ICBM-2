...
public class GYinGanQi extends GuiBase {
	public static final ResourceLocation TEXTURE = new ResourceLocation(
			ZhuYaoICBM.DOMAIN, ZhuYaoICBM.GUI_PATH + "gui_empty.png");
	...
	public void initGui() {
		...
		PacketDispatcher.sendPacketToServer(PacketManager.getPacket(
				ZhuYaoWanYi.CHANNEL, this.tileEntity, -1, true));
	...
	public void onGuiClosed() {
		super.onGuiClosed();
		PacketDispatcher.sendPacketToServer(PacketManager.getPacket(
				ZhuYaoWanYi.CHANNEL, this.tileEntity, -1, false));
	...
	protected void actionPerformed(GuiButton par1GuiButton) {
		if (par1GuiButton.id == 0) {
			...
			PacketDispatcher.sendPacketToServer(PacketManager.getPacket(
					ZhuYaoWanYi.CHANNEL, this.tileEntity, 2,
					this.tileEntity.mode));
		}
	}
	...
	public void keyTyped(char par1, int par2) {
		...
		try {
			Vector3 newMinCoord = new Vector3(
			...
			PacketDispatcher.sendPacketToServer(PacketManager.getPacket(
					ZhuYaoWanYi.CHANNEL, this.tileEntity, 4,
					this.tileEntity.minCoord.intX(),
					this.tileEntity.minCoord.intY(),
					this.tileEntity.minCoord.intZ()));
		} catch (Exception e) {
		}

		try {
			Vector3 newMaxCoord = new Vector3(
			...
			PacketDispatcher.sendPacketToServer(PacketManager.getPacket(
					ZhuYaoWanYi.CHANNEL, this.tileEntity, 5,
					this.tileEntity.maxCoord.intX(),
					this.tileEntity.maxCoord.intY(),
					this.tileEntity.maxCoord.intZ()));
		} catch (Exception e) {

		}

		try {
			short newFrequency = (short) Math.max(0,
					Short.parseShort(this.textFieldFreq.getText()));

			this.tileEntity.frequency = newFrequency;
			PacketDispatcher.sendPacketToServer(PacketManager.getPacket(
					ZhuYaoWanYi.CHANNEL, this.tileEntity, 3,
	...
	public void mouseClicked(int par1, int par2, int par3) {
		...
		this.textFieldminX.mouseClicked(par1 - containerWidth, par2
				- containerHeight, par3);
		this.textFieldminY.mouseClicked(par1 - containerWidth, par2
				- containerHeight, par3);
		this.textFieldminZ.mouseClicked(par1 - containerWidth, par2
				- containerHeight, par3);
		this.textFieldmaxX.mouseClicked(par1 - containerWidth, par2
				- containerHeight, par3);
		this.textFieldmaxY.mouseClicked(par1 - containerWidth, par2
				- containerHeight, par3);
		this.textFieldmaxZ.mouseClicked(par1 - containerWidth, par2
				- containerHeight, par3);
	...
	protected void drawForegroundLayer(int var2, int var3, float var1) {
		this.fontRenderer.drawString("\u00a77Proximity Detector", 48, 6,
				4210752);
		...
		this.fontRenderer.drawString(color + "Status: " + status, 12, 138,
				4210752);
		this.fontRenderer.drawString(
				ElectricityDisplay.getDisplay(
						this.tileEntity.getRequest(null) * 20,
						ElectricUnit.WATT)
						+ " "
						+ ElectricityDisplay.getDisplay(
								this.tileEntity.getVoltage(),
								ElectricUnit.VOLTAGE), 12, 150, 4210752);
	}
