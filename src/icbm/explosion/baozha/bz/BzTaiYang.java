package icbm.explosion.baozha.bz;
...
public class BzTaiYang extends BzGuang
{
	...
	{
		super.doExplode();
		this.worldObj.playSoundEffect(position.x, position.y, position.z, ZhuYaoICBM.PREFIX + "beamcharging", 4.0F, 0.8F);
	}
	...
	public void doPostExplode()
	{
		...
		{
			this.worldObj.playSoundEffect(position.x, position.y, position.z, ZhuYaoICBM.PREFIX + "powerdown", 4.0F, 0.8F);
			...
			if (this.canFocusBeam(this.worldObj, position) && this.thread.isComplete)
			{
					...
					if (chance > distanceFromCenter * 0.55)
					{
						...
						if ((blockID == 0 || blockID == Block.snow.blockID) && this.worldObj.getBlockMaterial(targetPosition.intX(), targetPosition.intY() - 1, targetPosition.intZ()).isSolid())
						{
							...
							else
							{
								...
								if (((ExTaiYang) ZhaPin.taiYang).createNetherrack && (blockID == Block.stone.blockID || blockID == Block.grass.blockID || blockID == Block.dirt.blockID) && this.worldObj.rand.nextFloat() > 0.75)
								{
				...
				this.worldObj.playSoundEffect(position.x + 0.5D, position.y + 0.5D, position.z + 0.5D, ZhuYaoICBM.PREFIX + "explosionfire", 6.0F, (1.0F + (worldObj.rand.nextFloat() - worldObj.rand.nextFloat()) * 0.2F) * 1F);
			}
