...
public class CommandRegistry {
	 ...
	 * 
	 * @param prefix
	 *            - what the command starts with for example /time
	 * @param cmd
	 *            - Cmd instance that will execute the command
	 */
	 ...
	 * 
	 * @param terminal
	 *            - The terminal, can be cast to TileEntity.
	 */
	public static void onCommand(EntityPlayer player, ITerminal terminal,
			String cmd) {
		if (cmd != null && cmd != "") {
					...
					if (command.getCommandPrefix().equalsIgnoreCase(args[0])) {
						if (!command.canMachineUse(terminal)) {
							...
							if (!command.canPlayerUse(player, terminal)) {
								...
								if (command.processCommand(player, terminal,
										args)) {
