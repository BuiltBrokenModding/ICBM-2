 ...
 * 
 */
public abstract class TileEntityTerminal extends TileEntityUniversalElectrical
		implements ISpecialAccess, IPacketReceiver, ITerminal {
	...
	public Packet getDescriptionPacket() {
		...
		this.writeToNBT(nbt);
		return PacketManager.getPacket(this.getChannel(), this,
				TerminalPacketType.DESCRIPTION_DATA.ordinal(), nbt);
	}
	...
	public void sendTerminalOutputToClients() {
		...
		Packet packet = PacketManager.getPacket(this.getChannel(), this,
				data.toArray());
	...
	public void sendCommandToServer(EntityPlayer entityPlayer, String cmdInput) {
		if (this.worldObj.isRemote) {
			Packet packet = PacketManager.getPacket(this.getChannel(), this,
					...
					entityPlayer.username, cmdInput);
			PacketDispatcher.sendPacketToServer(packet);
		}
	}

	@Override
	public void handlePacketData(INetworkManager network, int packetID,
			Packet250CustomPayload packet, EntityPlayer player,
			ByteArrayDataInput dataStream) {
		try {
			TerminalPacketType packetType = TerminalPacketType.values()[dataStream
					.readInt()];
			...
			case DESCRIPTION_DATA: {
				if (this.worldObj.isRemote) {
					...
					if (size > 0) {
						...
						this.readFromNBT(CompressedStreamTools
								.decompress(byteCode));
					}
				}
			...
			case GUI_COMMAND: {
				if (!this.worldObj.isRemote) {
					CommandRegistry.onCommand(this.worldObj
							.getPlayerEntityByName(dataStream.readUTF()), this,
							dataStream.readUTF());
			...
			case TERMINAL_OUTPUT: {
				if (this.worldObj.isRemote) {
					...
					List<String> oldTerminalOutput = new ArrayList(
							this.terminalOutput);
					...
					if (!this.terminalOutput.equals(oldTerminalOutput)
							&& this.terminalOutput.size() != oldTerminalOutput
									.size()) {
	...
	public boolean canUserAccess(String username) {
		return (this.getUserAccess(username).ordinal() >= AccessLevel.USER
				.ordinal());
	}
	...
	public void setScroll(int length) {
		this.scroll = Math.max(
				Math.min(length, this.getTerminalOuput().size()), 0);
	}
