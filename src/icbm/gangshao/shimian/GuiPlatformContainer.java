...
public abstract class GuiPlatformContainer extends GuiContainer {
	public static final ResourceLocation TEXTURE = new ResourceLocation(
			ZhuYaoICBM.DOMAIN, ZhuYaoICBM.GUI_PATH + "gui_base.png");

	protected static final int MAX_BUTTON_ID = 3;
	protected TPaoTaiZhan tileEntity;
	protected EntityPlayer entityPlayer;

	public GuiPlatformContainer(InventoryPlayer inventoryPlayer,
			TPaoTaiZhan tileEntity) {
	...
	public void initGui() {
		...
		// Terminal
		this.buttonList.add(new GuiButtonImage(0,
				(this.width - this.xSize) / 2 - 22,
				(this.height - this.ySize) / 2 + 0, 3));
		// Access
		this.buttonList.add(new GuiButtonImage(1,
				(this.width - this.xSize) / 2 - 22,
				(this.height - this.ySize) / 2 + 22, 0));
		// Ammunition
		this.buttonList.add(new GuiButtonImage(2,
				(this.width - this.xSize) / 2 - 22,
	...
	protected void actionPerformed(GuiButton button) {
		if (tileEntity.getTurret(false) != null) {
			...
			switch (button.id) {
			case 0: {
				this.entityPlayer.openGui(ZhuYaoGangShao.instance,
						CommonProxy.GUI_PLATFORM_TERMINAL_ID,
						this.tileEntity.worldObj, this.tileEntity.xCoord,
						this.tileEntity.yCoord, this.tileEntity.zCoord);
				break;
			}
			case 1: {
				this.entityPlayer.openGui(ZhuYaoGangShao.instance,
						CommonProxy.GUI_PLATFORM_ACCESS_ID,
						this.tileEntity.worldObj, this.tileEntity.xCoord,
						this.tileEntity.yCoord, this.tileEntity.zCoord);
				break;
			}
			case 2: {
				this.entityPlayer.openGui(ZhuYaoGangShao.instance,
						CommonProxy.GUI_PLATFORM_ID, this.tileEntity.worldObj,
	...
	protected void drawGuiContainerForegroundLayer(int x, int y) {
		if (this.tileEntity.getTurret(false) != null) {
			...
			this.fontRenderer.drawString("\u00a77" + title,
					(int) (this.xSize / 2 - title.length() * 2.5), 4, 4210752);

			/** Render Tool Tips */
			if (((GuiButtonImage) this.buttonList.get(0)).isIntersect(x, y)) {
				this.drawTooltip(x - this.guiLeft, y - this.guiTop + 10,
						"Terminal");
			} else if (((GuiButtonImage) this.buttonList.get(1)).isIntersect(x,
					y)) {
				this.drawTooltip(x - this.guiLeft, y - this.guiTop + 10,
						"Access");
			} else if (((GuiButtonImage) this.buttonList.get(2)).isIntersect(x,
					y)) {
				this.drawTooltip(x - this.guiLeft, y - this.guiTop + 10,
						"Ammunition");
			}/*
			 * else if (((GuiButtonImage) this.buttonList.get(3)).isIntersect(x,
			 * y)) { this.drawTooltip(x - this.guiLeft, y - this.guiTop + 10,
			 * "Protection"); }
			 */
		}
	}
	...
	public void drawTooltip(int x, int y, String... toolTips) {
		...
		if (toolTips != null) {
			...
			this.drawGradientRect(var6 - 3, var7 - 3, var6 + var5 + 3, var7
					+ var9 + 3, var10, var10);
			this.drawGradientRect(var6 - 4, var7 - 3, var6 - 3,
					var7 + var9 + 3, var10, var10);
			this.drawGradientRect(var6 + var5 + 3, var7 - 3, var6 + var5 + 4,
					var7 + var9 + 3, var10, var10);
			int var11 = 1347420415;
			int var12 = (var11 & 16711422) >> 1 | var11 & -16777216;
			this.drawGradientRect(var6 - 3, var7 - 3 + 1, var6 - 3 + 1, var7
					+ var9 + 3 - 1, var11, var12);
			this.drawGradientRect(var6 + var5 + 2, var7 - 3 + 1, var6 + var5
					+ 3, var7 + var9 + 3 - 1, var11, var12);
