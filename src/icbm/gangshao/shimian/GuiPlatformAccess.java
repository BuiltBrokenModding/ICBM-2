...
public class GuiPlatformAccess extends GuiPlatformBase implements IScroll {
	public static final ResourceLocation TEXTURE = new ResourceLocation(
			ZhuYaoICBM.DOMAIN, ZhuYaoICBM.GUI_PATH
					+ "gui_platform_terminal.png");
	...
	public void initGui() {
		...
		this.commandLine = new GuiTextField(this.fontRenderer, width + 12,
				height + 165, 135, 11);
		this.commandLine.setMaxStringLength(30);

		this.buttonList.add(new GuiButtonArrow(MAX_BUTTON_ID + 1, width + 151,
				height + 21, false));
		this.buttonList.add(new GuiButtonArrow(MAX_BUTTON_ID + 2, width + 151,
				height + 152, true));
	...
	protected void keyTyped(char character, int keycode) {
		...
		} else if (keycode == 208) // PAGE DOWN (no constant)
		{
			...
			this.tileEntity.sendCommandToServer(this.entityPlayer, command
					+ " " + username);
	...
	protected void mouseClicked(int x, int y, int type) {
		...
		if (type == 0) {
			Iterator<Entry<UserAccess, Vector2>> it = this.outputMap.entrySet()
					.iterator();
			...
			while (it.hasNext()) {
				...
				if (new Region2(minPos, maxPos).isIn(new Vector2(x
						- this.guiLeft, y - this.guiTop))) {
					...
					this.tileEntity.sendCommandToServer(this.entityPlayer,
	...
	protected void drawForegroundLayer(int x, int y, float var1) {
		...
		this.fontRenderer.drawString("\u00a77" + title,
				this.xSize / 2 - title.length() * 3, 4, 4210752);
	...
	public void drawConsole(int x, int y, int lines) {
		...
		for (int i = 0; i < lines; i++) {
			...
			if (currentLine < this.tileEntity.getUsers().size()
					&& currentLine >= 0) {
				UserAccess accesInfo = this.tileEntity.getUsers().get(
						currentLine);
				String line = accesInfo.username + " ("
						+ accesInfo.level.displayName + ")";

				if (line != null && line != "") {
					Vector2 drawPosition = new Vector2(x, SPACING * i + y);
					outputMap.put(accesInfo, drawPosition);
					this.fontRenderer.drawString(line, drawPosition.intX(),
	...
	public void setScroll(int length) {
		this.scroll = Math.max(
				Math.min(length, this.tileEntity.getUsers().size()), 0);
	}
