...
public class ClientProxy extends CommonProxy {
	...
	public void init() {
		...
		/** TileEntities */
		ClientRegistry
				.bindTileEntitySpecialRenderer(TQiang.class, new RQiang());
		ClientRegistry.bindTileEntitySpecialRenderer(TFanKong.class,
				new RFanKong());
		ClientRegistry.bindTileEntitySpecialRenderer(TCiGuiPao.class,
				new RCiGuiPao());
		ClientRegistry.bindTileEntitySpecialRenderer(TLeiShe.class,
				new RLeiShe());

		RenderingRegistry.registerEntityRenderingHandler(EJia.class,
				new REJia());
	...
	public Object getClientGuiElement(int ID, EntityPlayer player, World world,
		...
		if (tileEntity != null) {
			...
			case GUI_PLATFORM_ID:
				return new GuiPlatformSlots(player.inventory,
						((TPaoTaiZhan) tileEntity));
			case GUI_PLATFORM_TERMINAL_ID:
				return new GuiPlatformTerminal(player,
						((TPaoTaiZhan) tileEntity));
	...
	@Override
	public void renderBeam(World world, Vector3 position, Vector3 target,
			float red, float green, float blue, int age) {
		FMLClientHandler.instance().getClient().effectRenderer
				.addEffect(new FXBeam(world, position, target, red, green,
						blue, age));
	}
}
