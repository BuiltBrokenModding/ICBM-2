	...
	protected boolean onUpdateTask() {
		...
		if (this.tileEntity instanceof IAutoSentry) {
			...
			if (sentry.getTarget() == null
				...
				List<Entity> entities = this.tileEntity.worldObj
						.getEntitiesWithinAABB(Entity.class, bounds);
				Entity currentTarget = null;

				/**
				 * Try to look for the owner within range and attack the entity
				 * attacking the owner if possible.
				 * 
				 * for (Entity entity : entities) { if (entity instanceof
				 * EntityPlayer) { EntityPlayer player = (EntityPlayer) entity;
				 * AccessLevel level =
				 * this.tileEntity.getPlatform().getUserAccess(player.username);
				 * 
				 * /** Checks to see if this player is friendly. If so, attempt
				 * to protect the player.
				 * 
				 * if (level.ordinal() >= AccessLevel.USER.ordinal()) { if
				 * (player.get != null) { if
				...
				if (currentTarget == null) {
					...
					for (Entity entity : entities) {
						final double distance = this.tileEntity.getCenter()
								.distanceTo(new Vector3(entity));

						if (sentry.isValidTarget(entity)
				...
				if (currentTarget != null) {
					...
					return false;
				} else if (this.tileEntity.lastRotateTick > this.world.rand
						.nextInt(30) + 10) {
					this.tileEntity.rotateTo(
							this.world.rand.nextInt(360) - 180,
							this.world.rand.nextInt(30) - 10);
				}
