...
public class LookHelper {
	...
	public void lookAt(Vector3 target) {
		this.sentry.rotateTo(getYaw(sentry.getMuzzle(), target),
				getPitch(sentry.getMuzzle(), target));
	}

	public float[] getDeltaRotations(Vector3 target) {
		return new float[] { getYaw(sentry.getMuzzle(), target),
				getPitch(sentry.getMuzzle(), target) };
	}
	...
	public void lookAtEntity(Entity entity) {
		this.lookAt(Vector3.add(new Vector3(entity),
				new Vector3(0, entity.getEyeHeight(), 0)));
	}
	 ...
	 *            - xyz target
	 * @param allowedError
	 *            - amount these sentry can be off in degrees from target
	 ...
	 * 
	 * @param entity
	 *            - entity be used for the location
	 * @param allowedError
	 *            - amount these sentry can be off in degrees from target
	 * @return true if its with in error range
	 */
	public boolean isLookingAt(Entity entity, float allowedError) {
		return this.isLookingAt(
				new Vector3(entity.posX, entity.posY + entity.getEyeHeight(),
						entity.posZ), allowedError);
	}
	...
	public static float getPitch(Vector3 position, Vector3 target) {
		...
		double verticleDistance = MathHelper.sqrt_double(difference.x
				* difference.x + difference.z * difference.z);
		return -MathHelper.wrapAngleTo180_float((float) (Math.atan2(
				difference.y, verticleDistance) * 180.0D / Math.PI)
				+ PITCH_DISPLACEMENT);
	}
	...
	public boolean canPositionBeSeen(Vector3 target) {/*
													 ...
													 * = CalculationHelper.
													 * doCustomRayTrace
													 * (this.sentry.worldObj,
													 * this.sentry.getMuzzle(),
													 * rotationYaw,
													 * rotationPitch, true,
													 * this.
													 * sentry.getCenter().distanceTo
													 * (target));
													 * 
													 * if (mop != null) { if
													 * (mop.hitVec != null) {
													 ...
													 * return false;
													 */
		return this.sentry.worldObj.clip(this.sentry.getMuzzle().toVec3(),
				target.toVec3()) == null;
	}

	public boolean canEntityBeSeen(Entity entity) {
		Vector3 target = Vector3.add(new Vector3(entity),
				new Vector3(0, entity.getEyeHeight(), 0));
		/*
		 * float rotationYaw = getYaw(this.sentry.getCenter(), target); float
		 * rotationPitch = getPitch(this.sentry.getCenter(), target);
		 * MovingObjectPosition mop =
		 * CalculationHelper.doCustomRayTrace(this.sentry.worldObj,
		 * this.sentry.getCenter(), rotationYaw, rotationPitch, true,
		 * this.sentry.getCenter().distanceTo(target));
		 * 
		 * if (mop != null ) {System.out.println(mop.hitVec); return
		 * entity.equals(mop.entityHit); } return false;
		 */
