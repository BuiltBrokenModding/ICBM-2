...
public class TPaoTaiZhan extends TileEntityTerminal implements IInventory {
	...
	public void updateEntity() {
		...
		if (this.getTurret(false) != null) {
			this.electricityHandler.setMaxEnergyStored(Math.max(
					turret.getFiringRequest(), 0) * 2);
		} else {
		...
		if (!this.worldObj.isRemote) {
			for (int i = 0; i < UPGRADE_START_INDEX; i++) {
				...
				this.electricityHandler.receiveElectricity(ElectricItemHelper
						.dischargeItem(this.getStackInSlot(i),
	...
	public float receiveElectricity(ForgeDirection from,
			...
			if (UniversalElectricity.isVoltageSensitive) {
				if (receive.voltage > this.getVoltage()) {
					...
					if (turret != null && turret instanceof IHealthTile) {
						((IHealthTile) this.turret).onDamageTaken(
		...
		if ((this.prevWatts <= this.getRequest(null) && this.getEnergyStored() >= this
				.getRequest(null))
				&& !(this.prevWatts == this.getEnergyStored())) {
			this.worldObj.markBlockForUpdate(this.xCoord, this.yCoord,
					this.zCoord);
	...
	public float getRequest(ForgeDirection direction) {
		if (this.getTurret(false) != null) {
			if (this.getEnergyStored() < this.getTurret(false)
					.getFiringRequest()) {
	...
	public TPaoDaiBase getTurret(boolean getNew) {
		...
		if (getNew
				|| this.turret == null
				|| this.turret.isInvalid()
				|| !(new Vector3(this.turret).equals(position.clone()
						.modifyPositionFromSide(this.deployDirection)))) {
			TileEntity tileEntity = position.clone()
					.modifyPositionFromSide(this.deployDirection)
	...
	public boolean destroyTurret() {
		TileEntity ent = this.worldObj.getBlockTileEntity(this.xCoord
				+ deployDirection.offsetX, this.yCoord
				+ deployDirection.offsetY, this.zCoord
				+ deployDirection.offsetZ);
	...
	public boolean isRunning() {
		return (this.getTurret(false) != null && this.getEnergyStored() >= this
				.getTurret(false).getFiringRequest());
	}
	...
	public int getUpgradeCount(TurretUpgradeType type) {
		...
		for (int i = UPGRADE_START_INDEX; i < UPGRADE_START_INDEX
				+ TURRET_UPGADE_SLOTS; i++) {
			ItemStack itemStack = this.getStackInSlot(i);

			if (itemStack != null) {
				if (itemStack.getItem() instanceof ITurretUpgrade) {
					if (((ITurretUpgrade) itemStack.getItem())
							.getType(itemStack) == type) {
						count++;
					}
				}
			}
	...
	public boolean isItemValidForSlot(int slotID, ItemStack itemStack) {
		if (slotID < UPGRADE_START_INDEX
				&& itemStack.getItem() instanceof IAmmunition) {
			return true;
		}
	...
	public boolean addStackToInventory(ItemStack itemStack) {
		for (int i = 0; i < UPGRADE_START_INDEX; i++) {
			...
			if (checkStack == null) {
				...
				int inputStack = Math.min(checkStack.stackSize
						+ itemStack.stackSize, checkStack.getMaxStackSize())
