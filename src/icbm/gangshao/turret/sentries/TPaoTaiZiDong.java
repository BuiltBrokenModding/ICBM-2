...
public abstract class TPaoTaiZiDong extends TPaoDaiBase implements IAutoSentry {
	...
	public void onReceivePacket(int packetID, EntityPlayer player,
		...
		if (packetID == TurretPacketType.SHOT.ordinal()) {
			this.renderShot(new Vector3(dataStream.readDouble(), dataStream
					.readDouble(), dataStream.readDouble()));
	...
	public AxisAlignedBB getTargetingBox() {
		return AxisAlignedBB.getBoundingBox(xCoord - this.getDetectRange(),
				this.yCoord - 5, zCoord - this.getDetectRange(),
				xCoord + this.getDetectRange(), yCoord + 5,
	...
	public boolean isValidTarget(Entity entity) {
		if (entity != null) {
			if (!entity.isDead && !entity.isEntityInvulnerable()) {
				if (this.getCenter().distanceTo(new Vector3(entity)) <= this
						.getDetectRange()) {
					float[] rotations = this.lookHelper
							.getDeltaRotations(new Vector3(entity)
									.add(new Vector3(0, entity.getEyeHeight(),
											0)));

					if ((rotations[1] <= this.maxPitch && rotations[1] >= this.minPitch)
							|| this.allowFreePitch) {
	...
	protected boolean isAir(Entity entity) {
		return (entity instanceof IMob && entity instanceof EntityFlying)
				|| (entity instanceof IAATarget && ((IAATarget) entity)
						.canBeTargeted(this)) || entity instanceof EntityWither
						...
						&& (this.getPlatform()
								.provideElectricity(
										...
										ElectricityPack.getFromWatts(
												...
												this.getVoltage()), false)
								.getWatts() >= this.getFiringRequest())
						&& (this.getPlatform().hasAmmunition(
								this.projectileType) != null || this.projectileType == ProjectileType.UNKNOWN);
			}
		}
	...
	public void sendShotToClient(Vector3 position) {
		PacketManager.sendPacketToClients(PacketManager.getPacket(
				ZhuYaoGangShao.CHANNEL, this, TurretPacketType.SHOT.ordinal(),
				position.x, position.y, position.z, this.currentRotationPitch,
				this.currentRotationYaw), this.worldObj, new Vector3(this), 40);
	}
				...
				if (this.target instanceof EntityLiving) {
					...
					if (bullet.getType(ammoStack) == ProjectileType.CONVENTIONAL) {
						this.target.attackEntityFrom(
								TileDamageSource.doBulletDamage(this),
								bullet.getDamage());
					}
					...
					if (this.worldObj.rand.nextFloat() > 0.2) {
						...
						if (damage == -1
								&& this.worldObj.rand.nextFloat() > 0.7) {
				...
				if (fired && this.projectileType != ProjectileType.UNKNOWN) {
					if (this.getPlatform().useAmmunition(ammoStack)) {
						...
						if (this.getPlatform().getUpgradeCount(
								TurretUpgradeType.COLLECTOR) > 0) {
							if (this.getPlatform().addStackToInventory(
									ZhuYaoGangShao.bulletShell.copy())) {
								drop = false;
							}
						}
						...
						if (drop && this.worldObj.rand.nextFloat() < 0.9) {
							...
							EntityItem entityShell = new EntityItem(
									this.worldObj, spawnPos.x, spawnPos.y,
									spawnPos.z,
									ZhuYaoGangShao.bulletShell.copy());
	...
	public Vector3 getTargetPosition() {
		if (this.getTarget() != null) {
			return new Vector3(this.getTarget()).add(new Vector3(0, this
					.getTarget().getEyeHeight(), 0));
		}
