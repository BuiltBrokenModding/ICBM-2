...
public class TLeiShe extends TPaoTaiZiDong {
	...
	public void updateEntity() {
		...
		if (this.worldObj.isRemote) {
			this.barrelRotation = MathHelper
					.wrapAngleTo180_float(this.barrelRotation
							+ this.barrelRotationVelocity);
			this.barrelRotationVelocity = Math.max(
					this.barrelRotationVelocity - 0.1f, 0);
		}
	}
	...
	public void playFiringSound() {
		this.worldObj.playSoundEffect(this.xCoord, this.yCoord, this.zCoord,
				ZhuYaoICBM.PREFIX + "lasershot", 5F,
				1F - (this.worldObj.rand.nextFloat() * 0.2f));
	}
	...
	public void renderShot(Vector3 target) {
		...
		ZhuYaoGangShao.proxy.renderBeam(this.worldObj, Vector3.add(
				...
				CalculationHelper.getDeltaPositionFromRotation(
						this.currentRotationYaw - 6,
						this.currentRotationPitch * 1.4f).multiply(1.2)),
				target, 1, 0.4f, 0.4f, 5);
		ZhuYaoGangShao.proxy.renderBeam(this.worldObj, Vector3.add(
				center,
				CalculationHelper.getDeltaPositionFromRotation(
						this.currentRotationYaw + 6,
			...
			if (this.getPlatform() != null) {
				if (this.target instanceof EntityLiving) {
					...
					this.target.attackEntityFrom(
							TileDamageSource.doLaserDamage(this), 2);
