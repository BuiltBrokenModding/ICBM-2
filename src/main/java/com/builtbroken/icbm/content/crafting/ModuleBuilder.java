...
public class ModuleBuilder
{
    ...
    {
        String id = stack.getTagCompound().getString("moduleID");
        if(idToCLassMap.containsKey(id))
        {
            if(idToCLassMap.get(id) != null)
            {
                try
                {
                    return idToCLassMap.get(id).newInstance();
                }
                catch(InstantiationException e)
                {
                    ICBM.LOGGER.error("ModuleBuilder failed to find a default constructor for " + idToCLassMap.get(id));
