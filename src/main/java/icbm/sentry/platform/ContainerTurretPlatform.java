...
public class ContainerTurretPlatform extends Container
{
	...
	public ContainerTurretPlatform(InventoryPlayer inventoryPlayer, TileTurretPlatform tileEntity)
	{
		...
		for (int i = 0; i < 4; i++)
		{
			this.addSlotToContainer(new SlotTurret(tileEntity, i + TileTurretPlatform.UPGRADE_START_INDEX, 89 + i * 18, 77, ITurretUpgrade.class));
		}
	...
	public ItemStack transferStackInSlot(EntityPlayer entityPlayer, int slotID)
	{
		...
		if (slot != null && slot.getHasStack())
		{
			...
			{
				if (this.tileEntity.canUse(Nodes.INV_EDIT_NODE, entityPlayer))
				{
					if (itemStack.getItem() instanceof ItemAmmo || itemStack.getItem() instanceof IEnergyItem)
					{
						if (!this.mergeItemStack(itemStack, 0, TileTurretPlatform.UPGRADE_START_INDEX, false))
						{
							return null;
						}
					}
					...
					{
						if (!this.mergeItemStack(itemStack, TileTurretPlatform.UPGRADE_START_INDEX, this.tileEntity.containingItems.length, false))
						{
							return null;
						}
					}
