...
@BlockInfo(renderer = "icbm.sentry.turret.block.RenderSentry", tileEntity = "icbm.sentry.turret.block.TileSentry")
    ...
    public void getSubBlocks(int id, CreativeTabs creativeTab, List list)
    {
        for (Entry<String, Class<? extends Sentry>> entry : SentryRegistry.getSentryMap().entrySet())
        {
            if (entry.getValue() != null)
            {
                ...
                item_nbt.setString("unlocalizedName", entry.getKey());
                sentry_nbt.setString("id", SaveManager.getID(entry.getValue()));
                item_nbt.setCompoundTag("sentryTile", sentry_nbt);
                
                stack.setTagCompound(item_nbt);
                list.add(stack);
            }
        }
