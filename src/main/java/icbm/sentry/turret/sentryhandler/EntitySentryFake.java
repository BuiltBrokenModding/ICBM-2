 ...
 */
public class EntitySentryFake extends EntityLiving implements
	...
	public EntitySentryFake(TileSentry controller, boolean sit) {
		...
		this.setPosition(controller.xCoord, controller.yCoord,
				controller.zCoord);
	...
	protected void applyEntityAttributes() {
		...
		this.getEntityAttribute(SharedMonsterAttributes.maxHealth)
				.setAttribute(this.getMaxHealth());
	}
	...
	public void readSpawnData(ByteArrayDataInput data) {
		try {
			TileEntity entity = this.worldObj.getBlockTileEntity(
	...
	public void onUpdate() {
		...
		this.setPosition(this.sentryHost.xCoord + 0.5, this.sentryHost.yCoord,
				this.sentryHost.zCoord + 0.5);
	}
	...
	public void updateRiderPosition() {
		...
		if (this.riddenByEntity != null) {
			...
			if (this.sentryHost.getSentry() instanceof MountedSentry) {
				offset = ((MountedSentry) this.sentryHost.getSentry())
						.getRiderOffset();
			}
			offset.rotate(this.sentryHost.getYawServo().getRotation(),
					this.sentryHost.getPitchServo().getRotation());
			vec.add(offset);
			this.riddenByEntity.setPosition(vec.x, vec.y, vec.z);
			this.riddenByEntity.rotationYaw = this.sentryHost.getYawServo()
					.getRotation();
			this.riddenByEntity.rotationPitch = this.sentryHost.getPitchServo()
					.getRotation();
		}
	}
	...
	public double getMountedYOffset() {
		if (this.sentryHost.getSentry() instanceof MountedSentry)
			return ((MountedSentry) this.sentryHost.getSentry())
					.getRiderOffset().y;
		else
			return -0.5;
	}
	...
	public boolean interact(EntityPlayer player) {
		if (this.sentryHost != null && player != null) {
			Block block = Block.blocksList[this.worldObj.getBlockId(
					this.sentryHost.xCoord, this.sentryHost.yCoord,
					this.sentryHost.zCoord)];
			if (block != null) {
				return block.onBlockActivated(this.sentryHost.worldObj,
						this.sentryHost.xCoord, this.sentryHost.yCoord,
						this.sentryHost.zCoord, player, 0, 0, 0, 0);
			}
		}
		return false;
	}
