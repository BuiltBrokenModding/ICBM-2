        ...
        if (par2World.isRemote)
        {
            ...
            if (objectMouseOver != null)
            {
                ...
                if (!(tileEntity instanceof TileEntityLauncherPrefab))
                {
                    ...
                    {
                        PacketDispatcher.sendPacketToServer(PacketManager.getPacketWithID(ICBMExplosion.CHANNEL, ZhaPinPacketType.RADAR_GUN.ordinal(), objectMouseOver.blockX, objectMouseOver.blockY, objectMouseOver.blockZ));
            ...
            if (tileEntity != null)
            {
                if (tileEntity instanceof TileEntityLauncherScreen)
                {
                    ...
                    if (!savedCords.equals(new Vector3()))
                    {
                        ...
                        {
                            PacketDispatcher.sendPacketToServer(PacketManager.getPacket(ICBMExplosion.CHANNEL, missileLauncher, 2, savedCords.x, missileLauncher.getTarget().y, savedCords.z));
                ...
                else if (tileEntity instanceof TileEntityCruiseLauncher)
                {
                    ...
                    if (!savedCords.equals(new Vector3()))
                    {
                        ...
                        if (par3World.isRemote)
                        {
                            PacketDispatcher.sendPacketToServer(PacketManager.getPacket(ICBMExplosion.CHANNEL, missileLauncher, 2, savedCords.x, savedCords.y, savedCords.z));
                            par2EntityPlayer.addChatMessage("Coordinate information transfered!");
                        }
                    }
                    else
